...
public class WeightCommand extends Command {
      ...
      () -> {
        ...
        if (args.length == 6 && args[1].equals("calculate")) {
          try {
            ebMessage
              .editMessage(
                calculateWeight(args[2], args[3], args[4], args[5]).build()
              )
              .queue();
            return;
          ...
          ebMessage
            .editMessage(getPlayerWeight(args[1], args[2]).build())
            .queue();
          return;
  ...
  private EmbedBuilder calculateWeight(
    String skillAverage,
    String slayer,
    String catacombs,
    ...
    try {
      ...
      Weight calculatedWeight = new Weight(
        skillAverageD,
        slayerD,
        catacombsD,
        averageDungeonClassD
      );
      EmbedBuilder eb = defaultEmbed("Weight Calculator");
      eb.setDescription(
        "**Total Weight**: " +
        roundAndFormat(calculatedWeight.calculateTotalWeight())
      );
      eb.addField(
        "Slayer Weight",
        roundAndFormat(calculatedWeight.calculateSlayerWeight()),
        false
      );
      eb.addField(
        "Skills Weight",
        roundAndFormat(calculatedWeight.calculateSkillsWeight()),
        false
      );
      eb.addField(
        "Dungeons Weight",
        roundAndFormat(calculatedWeight.calculateDungeonsWeight()),
        false
      );
      return eb;
  ...
  private EmbedBuilder getPlayerWeight(String username, String profileName) {
    ...
    if (player.isValid()) {
      ...
      eb.setDescription(
        "**Total Weight**: " + roundAndFormat(playerWeight.getTotalWeight())
      );
      eb.addField(
        "Slayer Weight",
        roundAndFormat(playerWeight.getSlayerWeight()),
        false
      );
      eb.addField(
        "Skills Weight",
        roundAndFormat(playerWeight.getSkillsWeight()),
        false
      );
      eb.addField(
        "Dungeons Weight",
        roundAndFormat(playerWeight.getDungeonsWeight()),
        false
      );
      return eb;
    }
