...
public class SkillsWeight {
  ...
  public void addSkillWeight(
    ...
    if (currentSkillXp != -1) {
      ...
      double level =
        player.getSkill(skillName).skillLevel +
        player.getSkill(skillName).progressToNext;
  ...
  public void addSkillWeight(double skillAverage, double exponent) {
    this.totalSkillWeight +=
      (
        8 *
        (
          Math.pow(skillAverage * 10, 0.5 + exponent + (skillAverage / 100)) /
          1250
        )
      );
  }
