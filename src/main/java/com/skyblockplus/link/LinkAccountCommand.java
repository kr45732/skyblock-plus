...
public class LinkAccountCommand extends Command {
    ...
    new Thread(
      () -> {
        ...
        if (args.length == 2) {
          ebMessage
            .editMessage(
              linkAccount(args[1], event.getAuthor(), event.getGuild()).build()
            )
            .queue();
          return;
    ...
    if (playerInfo != null) {
      ...
      if (database.addLinkedUser(toAdd) == 200) {
        try {
          if (
            !higherDepth(
              database.getVerifySettings(guild.getId()),
              "verifiedNickname"
            )
              .getAsString()
              .equalsIgnoreCase("none")
          ) {
            String nicknameTemplate = higherDepth(
              database.getVerifySettings(guild.getId()),
              "verifiedNickname"
            )
              .getAsString();
            nicknameTemplate =
              nicknameTemplate.replace("[IGN]", playerInfo.minecraftUsername);
            if (nicknameTemplate.contains("[GUILD_RANK]")) {
              try {
                List<String> settingsGuildId = database
                  .getAllGuildRoles(guild.getId())
                  .stream()
                  .map(o1 -> o1.getGuildId())
                ...
                if (
                  settingsGuildId.contains(
                    higherDepth(playerGuild, "_id").getAsString()
                  )
                ) {
                  JsonArray guildMembers = higherDepth(playerGuild, "members")
                    .getAsJsonArray();
        ...
        try {
          JsonArray verifyRoles = higherDepth(
            database.getVerifySettings(guild.getId()),
            "verifiedRoles"
          )
            .getAsJsonArray();
          for (JsonElement verifyRole : verifyRoles) {
            try {
              guild
                .addRoleToMember(
                  user.getId(),
                  guild.getRoleById(verifyRole.getAsString())
                )
                .queue();
            } catch (Exception e) {
              System.out.println(
                "== Stack Trace (linkAccount - add role inside for) =="
              );
              e.printStackTrace();
            }
          }
          ...
          System.out.println(
            "== Stack Trace (linkAccount - add role outside for) =="
          );
          e.printStackTrace();
        }
  ...
  private EmbedBuilder getLinkedAccount(CommandEvent event) {
    JsonElement userInfo = database.getLinkedUserByDiscordId(
    ...
    try {
      ...
      return defaultEmbed("Error")
        .setDescription("`" + event.getAuthor().getAsTag() + "` is not linked");
    }
  }
}
