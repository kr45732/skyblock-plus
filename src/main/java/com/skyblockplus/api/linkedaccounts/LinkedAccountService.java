...
public class LinkedAccountService {
  ...
  public ResponseEntity<?> getByDiscordId(String discordId) {
    if (accountByDiscordIdExists(discordId)) {
      return new ResponseEntity<>(
        settingsRepository.findByDiscordId(discordId),
        HttpStatus.OK
      );
    }
  ...
  public ResponseEntity<?> getByMinecraftUuid(String minecraftUuid) {
    if (accountByMinecraftUuidExists(minecraftUuid)) {
      return new ResponseEntity<>(
        settingsRepository.findByMinecraftUuid(minecraftUuid),
        HttpStatus.OK
      );
    }
    return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
  ...
  public ResponseEntity<?> getByMinecraftUsername(String minecraftUsername) {
    if (accountByMinecraftUsernameExists(minecraftUsername)) {
      return new ResponseEntity<>(
        settingsRepository.findByMinecraftUsername(minecraftUsername),
        HttpStatus.OK
      );
    }
    return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
  ...
  private boolean accountByMinecraftUsernameExists(String minecraftUsername) {
    return (
      settingsRepository.findByMinecraftUsername(minecraftUsername) != null
    );
  }
  ...
  public ResponseEntity<HttpStatus> addNewLinkedAccount(
    LinkedAccountModel linkedAccountModel
  ) {
    if (
      settingsRepository.existsByMinecraftUsername(
        linkedAccountModel.getMinecraftUsername()
      )
    ) {
      deleteByMinecraftUsername(linkedAccountModel.getMinecraftUsername());
    } else if (
      settingsRepository.existsByMinecraftUuid(
        linkedAccountModel.getMinecraftUuid()
      )
    ) {
      deleteByMinecraftUuid(linkedAccountModel.getMinecraftUuid());
    } else if (
      settingsRepository.existsByDiscordId(linkedAccountModel.getDiscordId())
    ) {
