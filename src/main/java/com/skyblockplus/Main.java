...
public class Main {
	...
	public static void main(String[] args) throws LoginException, IllegalArgumentException {
		...
		Main.client = new CommandClientBuilder()
				...
				.setPrefixFunction(
						event -> event.isFromGuild() ? getGuildPrefix(event.getGuild().getId()) : DEFAULT_PREFIX)
				.setListener(
						new CommandListener() {
							@Override
		...
		slashCommandClient = new SlashCommandClient()
				...
				.addSlashCommands(
						...
						new PartySlashCommand(), new SettingsSlashCommand());
		...
		jda.getPresence()
				.setActivity(Activity.watching(DEFAULT_PREFIX + "help in " + jda.getGuilds().size() + " servers"));
...
/*
 * worker: java $JAVA_OPTS -XX:+UnlockExperimentalVMOptions
 * -XX:+UseCGroupMemoryLimitForHeap -XX:+UseStringDeduplication -jar
 * build/libs/SkyblockPlus-0.0.1.jar
 * web: java -Dserver.port=${PORT} -XX:+UnlockExperimentalVMOptions
 * -XX:+UseCGroupMemoryLimitForHeap -XX:+UseStringDeduplication -jar
 * build/libs/SkyblockPlus-0.0.1.jar
 */
