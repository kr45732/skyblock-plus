...
public class QuickSetupTestCommand extends Command {
      ...
      () -> {
        ...
        if (args.length >= 4) {
          if (args[1].equals("roles")) {
            ebMessage
              .editMessage(setRoleSettings(args[2], args[3], event).build())
              .queue();
            return;
            ...
            switch (args[2]) {
              ...
              case "apply_cache":
                ebMessage
                  .editMessage(deleteServerApplyCache(args[3], args[4]).build())
                  .queue();
                return;
              case "skyblock_event":
                ebMessage
                  .editMessage(deleteSkyblockEvent(args[3]).build())
                  .queue();
                return;
            }
          }
  ...
  private EmbedBuilder deleteSkyblockEvent(String serverId) {
    return defaultEmbed(
      "API returned response code " +
      database.updateSkyblockEventSettings(serverId, new SbEvent())
    );
  }
  ...
  private EmbedBuilder deleteServerApplyCache(String serverId, String name) {
    if (database.getServerSettings(serverId) != null) {
      return defaultEmbed(
        "API returned response code " +
        database.deleteApplyCacheSettings(serverId, name)
      );
    }
  ...
  private EmbedBuilder deleteServer(String serverId) {
    if (database.getServerSettings(serverId) != null) {
      return defaultEmbed(
        "API returned response code " + database.deleteServerSettings(serverId)
      );
    }
    return defaultEmbed("Error updating settings");
  }

  private EmbedBuilder setRoleSettings(
    String roleName,
    String json,
    CommandEvent event
  ) {
    try {
      JsonElement jsonElement = new Gson()
        .toJsonTree(new Gson().fromJson(json, RoleModel.class));
      if (
        higherDepth(
