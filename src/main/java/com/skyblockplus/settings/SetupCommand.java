...
public class SetupCommand extends Command {
    ...
    protected void execute(CommandEvent event) {
        ...
        String verify = "**Automated verify overview**:\n1) When a user reacts to the verify message, a new channel is created.\n2) They are then prompted for their in game minecraft username.\n3) This is then compared with their linked discord account and if matching they will recive the verifed role and the channel will be deleted.\n• NOTE: By default, automatic verify is disabled. In order to enable automatic verify you must specify the following settings.\n\n- `"
                ...
                + BOT_PREFIX
                + "settings verify prefix [prefix]` - the prefix that new verfiy channels will start with. For example a prefix of __verify__ and a user named __CrypticPlasma__ would result in a new channel called __verify-crypticplasma__.\n- `"
                + BOT_PREFIX
                + "settings verify category [category id]` - the category where new verfiy channels will be put. In order to get the ID of a category, type `"
                + BOT_PREFIX
