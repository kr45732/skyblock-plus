...
public class SettingsCommand extends Command {
	...
	private EmbedBuilder setOneLevelRole(String roleName, String roleMention) {
		if (!isOneLevelRole(roleName)) {
			return defaultEmbed("Error")
				.setDescription(
					"This role does is not a one level role. Use `" + BOT_PREFIX + "settings roles add [roleName] [value] [@role]` instead"
				);
		}
	...
	private EmbedBuilder removeApplyRequirement(String name, String reqNumber) {
		...
		try {
			...
			return defaultEmbed("Error")
				.setDescription("Invalid requirement number. Run `" + BOT_PREFIX + "settings apply` to see the current apply requirements");
		}
	}
