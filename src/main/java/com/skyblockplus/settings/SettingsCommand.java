...
public class SettingsCommand extends Command {
  ...
  protected void execute(CommandEvent event) {
    new Thread(
      () -> {
        ...
        JsonElement currentSettings = database.getServerSettings(
        ...
        if (higherDepth(currentSettings, "serverId") == null) {
          ...
          currentSettings =
            database.getServerSettings(event.getGuild().getId());
        }

        if ((args.length == 3) && (args[1].equals("delete"))) {
          if (args[2].equals("--confirm")) {
            if (
              database.deleteServerSettings(event.getGuild().getId()) == 200
            ) {
              ebMessage
                .editMessage(
                  defaultEmbed("Success")
                    .setDescription("Server settings deleted")
                    .build()
                )
                .queue();
            } else {
              ebMessage
                .editMessage(
                  defaultEmbed("Error")
                    .setDescription("Error deleting server settings")
                    .build()
                )
                .queue();
            }
        ...
        if (args.length == 1) {
          ...
          if (higherDepth(currentSettings, "automatedVerify") != null) {
            eb.addField(
              ...
              "Use `" +
              BOT_PREFIX +
              "settings verify` to see the current verify settings",
              false
            );
          } else {
          ...
          eb.addField(
            "Apply Settings",
            "Use `" + BOT_PREFIX + "settings apply` to see all apply settings",
            false
          );
          ...
          if (higherDepth(currentSettings, "automatedRoles") != null) {
            eb.addField(
              ...
              "Use `" +
              BOT_PREFIX +
              "settings roles` to see the current roles settings",
              false
            );
          } else {
          ...
          if (higherDepth(currentSettings, "automaticGuildRoles") != null) {
            eb.addField(
              ...
              "Use `" +
              BOT_PREFIX +
              "settings guild` to see the current guild role settings",
              false
            );
          } else {
          ...
          if (args.length == 2) {
            ...
            if (higherDepth(currentSettings, "automatedRoles") != null) {
              ...
              getCurrentRolesSettings(
                higherDepth(currentSettings, "automatedRoles")
              )
                .build()
          ...
          content.split(" ", 5).length >= 2 &&
          content.split(" ", 5)[1].equals("apply")
        ) {
          ...
          if (args.length == 2) {
            ...
            eb.addField(
              "Automatic Apply One",
              (
                higherDepth(currentSettings, "automatedApplicationOne.name") !=
                  null
                  ? "Name: " +
                  higherDepth(currentSettings, "automatedApplicationOne.name")
                    .getAsString() +
            ...
            eb.addField(
              "Automatic Apply Two",
              (
                higherDepth(currentSettings, "automatedApplicationTwo.name") !=
                  null
                  ? "Name: " +
                  higherDepth(currentSettings, "automatedApplicationTwo.name")
                    .getAsString() +
            ...
            eb = defaultEmbed("Settings for " + event.getGuild().getName());
            JsonElement applySettings = database.getApplySettings(
              event.getGuild().getId(),
              args[2]
            );
                  ...
                  if (allowApplyEnable(args[2])) {
                    ...
                    eb =
                      defaultEmbed("Error", null)
                        .setDescription(
                          "All other apply settings must be set before " +
                          "enabling apply!"
                        );
                  }
                ...
                case "requirements":
                  ...
                  if (args.length >= 6) {
                    if (args[4].equals("add")) {
                      eb =
                        addApplyRequirement(args[2], content.split(" ", 6)[5]);
                    } else if (args[4].equals("remove")) {
                      eb = removeApplyRequirement(args[2], args[5]);
                    } else {
                      eb =
                        defaultEmbed("Error").setDescription("Invalid setting");
                    }
                  } else {
                    eb =
                      defaultEmbed("Error").setDescription("Invalid setting");
          ...
          content.split(" ", 4).length >= 2 &&
          content.split(" ", 4)[1].equals("verify")
        ) {
          args = content.split(" ", 4);
          if (args.length == 2) {
            eb =
              defaultEmbed("Settings for " + event.getGuild().getName(), null);
            if (higherDepth(currentSettings, "automatedVerify") != null) {
              eb.addField(
                "Verify Settings",
                getCurrentVerifySettings(
                  higherDepth(currentSettings, "automatedVerify")
                ),
                false
              );
            } else {
            ...
            if (args[2].equals("enable")) {
              if (allowVerifyEnable()) {
                ...
                eb =
                  defaultEmbed("Error", null)
                    .setDescription(
                      "All other verify settings must be set before " +
                      "enabling verify!"
                    );
              }
            } else if (args[2].equals("disable")) {
              ...
              eb =
                defaultEmbed("Error", null).setDescription("Invalid setting");
            }
          } else if (args.length == 4) {
            switch (args[2]) {
              case "message":
            ...
            content.split(" ").length == 5 && args[2].equals("role")
          ) {}
          ...
          if (args.length == 2) {
            ...
            eb.addField(
              "Guild Roles One",
              (
                higherDepth(currentSettings, "automaticGuildRolesOne.name") !=
                  null
                  ? "Name: " +
                  higherDepth(currentSettings, "automaticGuildRolesOne.name")
                    .getAsString() +
            ...
            eb.addField(
              "Guild Roles Two",
              (
                higherDepth(currentSettings, "automaticGuildRolesTwo.name") !=
                  null
                  ? "Name: " +
                  higherDepth(currentSettings, "automaticGuildRolesTwo.name")
                    .getAsString() +
            ...
            if (guildRoleSettings != null && !guildRoleSettings.isJsonNull()) {
              eb.addField(
                "Guild Role Settings",
                getCurrentGuildRoleSettings(guildRoleSettings),
                false
              );
            } else {
              eb.addField(
  ...
  private EmbedBuilder deleteApplyGuild(String name) {
    if (database.getApplySettings(event.getGuild().getId(), name) != null) {
      int responseCode = database.removeApplySettings(
        event.getGuild().getId(),
        name
      );
      if (responseCode == 200) {
        return defaultEmbed("Settings for " + event.getGuild().getName())
          .setDescription(
            "Apply settings with name `" + name + "` was deleted"
          );
      }

      return defaultEmbed("Error")
        .setDescription("API returned response code " + responseCode);
    }
  ...
  private EmbedBuilder removeVerifyRole(String roleMention) {
    ...
    try {
      verifyRole =
        event.getGuild().getRoleById(roleMention.replaceAll("[<@&>]", ""));
    ...
    JsonArray currentVerifyRoles = higherDepth(
      database.getVerifySettings(event.getGuild().getId()),
      "verifiedRoles"
    )
      .getAsJsonArray();
    ...
    int responseCode = database.updateVerifyRolesSettings(
      event.getGuild().getId(),
      currentVerifyRoles
    );
    ...
    EmbedBuilder eb = defaultEmbed(
      "Settings for " + event.getGuild().getName()
    );
    return eb.setDescription(
      "**Removed verify role:** " + verifyRole.getAsMention()
    );
  }
  ...
  private EmbedBuilder addVerifyRole(String roleMention) {
    ...
    try {
      ...
      if ((verifyRole.isPublicRole() || verifyRole.isManaged())) {
        return defaultEmbed("Error")
          .setDescription("Role cannot be managed or @everyone");
      }
    } catch (Exception e) {
      return defaultEmbed("Invalid Role", null);
    }
    ...
    if (currentVerifyRoles.size() == 5) {
      return defaultEmbed(
        "You have reached the max number of verify roles (5/5)"
      );
    }
    ...
    return eb.setDescription(
      "**Verify role added:** " + verifyRole.getAsMention()
    );
  }
  ...
  private String getCurrentGuildRoleSettings(JsonElement currentSettings) {
    ...
    ebFieldString +=
      "**" + displaySettings(currentSettings, "enableGuildRole") + "**";
    ebFieldString +=
    ...
    try {
      for (JsonElement guildRank : higherDepth(currentSettings, "guildRanks")
        .getAsJsonArray()) {
        guildRanksString
          .append("\n• ")
    ...
    ebFieldString +=
      guildRanksString.length() > 0
        ? guildRanksString.toString()
        : "\n• No guild ranks set";
  ...
  private EmbedBuilder setGuildRoleEnable(String name, String enable) {
    JsonObject currentSettings = database
      .getGuildRoleSettings(event.getGuild().getId(), name)
      .getAsJsonObject();
    if (
    ...
    int responseCode = database.updateGuildRoleSettings(
      event.getGuild().getId(),
      currentSettings
    );
    if (responseCode != 200) {
      return defaultEmbed("Error", null)
        .setDescription("API returned response code " + responseCode);
    }
    ...
    eb.setDescription(
      "Guild role " + (enable.equals("true") ? "enabled" : "disabled")
    );
    return eb;
  }
  ...
  private EmbedBuilder setGuildRankEnable(String name, String enable) {
    ...
    if (
      ...
      return defaultEmbed("Error")
        .setDescription("The guild name and a guild rank must be set");
    }
    ...
    eb.setDescription(
      "Guild ranks " + (enable.equals("true") ? "enabled" : "disabled")
    );
    return eb;
  }

  private EmbedBuilder addGuildRank(
    String name,
    String rankName,
    String roleMention
  ) {
    ...
    JsonArray guildRanks = higherDepth(guildJson, "guild.ranks")
      .getAsJsonArray();
    ...
    for (JsonElement guildRank : guildRanks) {
      ...
      if (
        higherDepth(guildRank, "name")
          .getAsString()
          .equalsIgnoreCase(rankName.replace("_", " "))
      ) {
        JsonArray currentGuildRanks = currentSettings
          .get("guildRanks")
          .getAsJsonArray();

        for (JsonElement level : currentGuildRanks) {
          if (
            higherDepth(level, "minecraftRoleName")
              .getAsString()
              .equalsIgnoreCase(rankName)
          ) {
        ...
        Gson gson = new Gson();
        currentGuildRanks.add(
          gson.toJsonTree(
            new GuildRank(rankName.toLowerCase(), guildRankRole.getId())
          )
        );
        ...
        eb.setDescription(
          "**Guild rank added:** " +
          higherDepth(guildRank, "name").getAsString() +
          " - " +
    ...
    return defaultEmbed("Invalid guild rank")
      .setDescription(
        (
          guildRanksString.length() > 0
            ? "Valid guild ranks are: " + guildRanksString
  ...
  private EmbedBuilder removeGuildRank(String name, String rankName) {
    ...
    for (JsonElement guildRank : currentGuildRanks) {
      if (
        ...
        JsonArray currentGuildRanksTemp = currentSettings
          .get("guildRanks")
          .getAsJsonArray();
  ...
  private EmbedBuilder setGuildRoleName(String name, String roleMention) {
    try {
      Role verifyGuildRole = event
        .getGuild()
        .getRoleById(roleMention.replaceAll("[<@&>]", ""));
      if (!(verifyGuildRole.isPublicRole() || verifyGuildRole.isManaged())) {
        ...
        eb.setDescription(
          "**Guild role set to:** " + verifyGuildRole.getAsMention()
        );
        return eb;
      }
    } catch (Exception ignored) {}
  ...
  private EmbedBuilder createGuildRoles(String name) {
    if (name.length() > 25) {
      return defaultEmbed("Error")
        .setDescription("Name cannot be more than 25 letters");
    }

    List<GuildRole> currentGuildRoles = database.getAllGuildRoles(
      event.getGuild().getId()
    );
    ...
    if (currentGuildRoles.size() == 2) {
      return defaultEmbed("Error")
        .setDescription("You can reached the max amount of apply guilds (2/2)");
    }
  ...
  private EmbedBuilder getCurrentRoleSettings(String roleName) {
    ...
    if (rolePageMap.containsKey(roleName)) {
      ...
      currentRoleSettings
        .build()
        .paginate(event.getChannel(), rolePageMap.get(roleName));
      return null;
    } else {
      try {
  ...
  private CustomPaginator.Builder getCurrentRolesSettings(
    ...
    for (int i = 1; i < roleNames.size(); i++) {
      pageNumbers
        ...
        .append(i + 1)
        .append(":** ")
        .append(roleNames.get(i));
    }

    paginateBuilder.addItems(
      "**Automated Roles " +
      (
        higherDepth(rolesSettings, "enable").getAsString().equals("true")
          ? "Enabled"
          : "Disabled"
      ) +
      "**" +
    ...
    for (String roleName : roleNames) {
      ...
      if (higherDepth(currentRoleSettings, "enable") == null) {
        database.updateRoleSettings(
          event.getGuild().getId(),
          roleName,
          new Gson().toJsonTree(new RoleModel())
        );
        currentRoleSettings =
          database.getRoleSettings(event.getGuild().getId(), roleName);
      }
        ...
        case "guild_member":
          {
            ebFieldString
              ...
              .append(BOT_PREFIX)
              .append(
                "settings roles add guild_member skyblock_forceful @sbf guild member`\n"
              );
            break;
          }
        case "sven":
        ...
        case "fairy_souls":
          {
            ebFieldString
              ...
              .append(BOT_PREFIX)
              .append(
                "settings roles add fairy_souls 50 @50 souls collected`\n"
              );
            break;
          }
        case "slot_collector":
          {
            ebFieldString
              .append(
                "**Number of minion slots excluding upgrades (__not fully working__)**\nExample: `"
              )
              .append(BOT_PREFIX)
              .append(
                "settings roles add slot_collector 24 @maxed minion slots`\n"
              );
            break;
          }
        case "pet_enthusiast":
        ...
        case "dungeon_secrets":
          {
            ebFieldString
              ...
              .append(BOT_PREFIX)
              .append(
                "settings roles add dungeon_secrets 25000 @secret sweat`\n"
              );
            break;
          }
      }
      ...
      if (roleName.equals("guild_ranks")) {
        if (
          higherDepth(currentRoleSettings, "levels").getAsJsonArray().size() ==
          0
        ) {
          ebFieldString.append("\n• No ranks added");
        } else {
          for (JsonElement roleLevel : higherDepth(
            currentRoleSettings,
            "levels"
          )
            .getAsJsonArray()) {
        ...
        try {
          ebFieldString
            ...
            .append(
              "<@&" +
              higherDepth(
                higherDepth(currentRoleSettings, "levels")
                  .getAsJsonArray()
                  .get(0),
                "roleId"
              )
        ...
        ebFieldString.append(
          higherDepth(currentRoleSettings, "stackable")
              .getAsString()
              .equals("true")
            ? "\n• Stackable"
            : "\n• Not stackable"
        ...
        if (roleName.equals("guild_member")) {
          for (JsonElement roleLevel : higherDepth(
            ...
            ebFieldString
              ...
              .append(" - ")
              .append(
                "<@&" + higherDepth(roleLevel, "roleId").getAsString() + ">"
              );
          }
        } else {
          for (JsonElement roleLevel : higherDepth(
    ...
    return paginateBuilder.setPaginatorExtras(
      new PaginatorExtras().setTitles(pageTitles)
    );
  }

  private boolean allowRolesEnable() {
    JsonObject currentSettings = database
      .getRolesSettings(event.getGuild().getId())
      .getAsJsonObject();
  ...
  private EmbedBuilder setRolesEnable(String enable) {
    if (enable.equalsIgnoreCase("true") || enable.equalsIgnoreCase("false")) {
      JsonObject newRolesJson = database
        .getRolesSettings(event.getGuild().getId())
        .getAsJsonObject();
      newRolesJson.remove("enable");
      newRolesJson.addProperty("enable", enable);
      int responseCode = database.updateRolesSettings(
        event.getGuild().getId(),
        newRolesJson
      );
      if (responseCode != 200) {
  ...
  private EmbedBuilder setRoleEnable(String roleName, String enable) {
    ...
    try {
      currentRoleSettings =
        database
          .getRoleSettings(event.getGuild().getId(), roleName)
          .getAsJsonObject();
    } catch (Exception ignored) {}
    ...
    if (currentRoleSettings.get("levels").getAsJsonArray().size() != 0) {
      ...
      int responseCode = database.updateRoleSettings(
        event.getGuild().getId(),
        roleName,
        currentRoleSettings
      );
  ...
  private EmbedBuilder addRoleLevel(
    String roleName,
    String roleValue,
    String roleMention
  ) {
    ...
    if (roleName.equals("guild_member")) {
      ...
      JsonObject newRoleSettings = database
        .getRoleSettings(event.getGuild().getId(), "guild_ranks")
        .getAsJsonObject();
      ...
      if (currentLevels.size() >= 2) {
        return defaultEmbed("Error")
          .setDescription(
            "This role has reached the max limit of levels (2/2)"
          );
      }
      JsonElement guildRoleSettings = database.getGuildRoleSettings(
        event.getGuild().getId(),
      ...
      if (
        ...
        higherDepth(guildRoleSettings, "enableGuildRanks")
          .getAsString()
        ...
        Gson gson = new Gson();
        currentLevels.add(
          gson.toJsonTree(new RoleObject(roleValue, roleValue))
        );
        ...
        return defaultEmbed("Settings for " + event.getGuild().getName(), null)
          .setDescription(
            "Added guild ranks for guild roles with name `" + roleValue + "`"
          );
      }

      return defaultEmbed("Error")
        .setDescription("Invalid guild role name or guild ranks not enabled");
    } else if (isOneLevelRole(roleName)) {
      return defaultEmbed(
      ...
      try {
        ...
        return defaultEmbed("Error")
          .setDescription("Role value must be an integer");
      }
    }
    ...
    if (currentLevels.size() >= 5) {
      return defaultEmbed("Error")
        .setDescription("This role has reached the max limit of levels (5/5)");
    }
    ...
    if (!roleName.equals("guild_member")) {
      RoleObject[] temp = gson.fromJson(
        currentLevels,
        new TypeToken<RoleObject[]>() {}.getType()
      );
      Arrays.sort(
        temp,
    ...
    return defaultEmbed("Settings for " + event.getGuild().getName(), null)
      .setDescription(
        roleName + " " + roleValue + " set to " + role.getAsMention()
      );
  }
  ...
  private EmbedBuilder removeRoleLevel(String roleName, String value) {
    ...
    JsonArray currentLevels = currentRoleSettings
      .get("levels")
      .getAsJsonArray();
    for (JsonElement level : currentLevels) {
  ...
  private EmbedBuilder setRoleStackable(String roleName, String stackable) {
    if (isOneLevelRole(roleName) || roleName.equals("guild_ranks")) {
      return defaultEmbed("Error")
        .setDescription("This role does not support stacking");
    }
    JsonObject currentRoleSettings;
    try {
      currentRoleSettings =
    ...
    eb.setDescription(
      ...
      roleName +
      " role:** " +
      (stackable.equalsIgnoreCase("true") ? "stackable" : "not stackable")
    );
    return eb;
  }
  ...
  private String getCurrentVerifySettings(JsonElement verifySettings) {
    ...
    ebFieldString +=
      "\n**• React Message Text:** " +
      displaySettings(verifySettings, "messageText");
    ebFieldString +=
      "\n**• React Message Channel:** " +
      displaySettings(verifySettings, "messageTextChannelId");
    ebFieldString +=
      "\n**• Verified Role(s):** " +
      displaySettings(verifySettings, "verifiedRoles");
    ebFieldString +=
      "\n**• Nickname Template:** " +
      displaySettings(verifySettings, "verifiedNickname");
    return ebFieldString;
  }

  private boolean allowVerifyEnable() {
    JsonObject currentSettings = database
      .getVerifySettings(event.getGuild().getId())
      .getAsJsonObject();
  ...
  private EmbedBuilder setVerifyMessageText(String verifyText) {
    if (verifyText.length() > 0) {
      if (EmojiParser.parseToAliases(verifyText).length() > 1500) {
        return defaultEmbed("Error", null)
          .setDescription("Text cannot be longer than 1500 letters!");
      }

      int responseCode = updateVerifySettings(
        "messageText",
  ...
  private EmbedBuilder setVerifyMessageTextChannelId(String textChannel) {
    try {
      TextChannel verifyMessageTextChannel = event
        .getGuild()
        .getTextChannelById(textChannel.replaceAll("[<#>]", ""));
      int responseCode = updateVerifySettings(
        "messageTextChannelId",
        verifyMessageTextChannel.getId()
      );
      if (responseCode != 200) {
        return defaultEmbed("Error", null)
          .setDescription("API returned response code " + responseCode);
      ...
      eb.setDescription(
        "**Verify text channel set to:** " +
        verifyMessageTextChannel.getAsMention()
      );
      return eb;
    } catch (Exception ignored) {}
  ...
  private EmbedBuilder setVerifyNickname(String nickname) {
    if (!nickname.contains("[IGN]")) {
      ...
      return defaultEmbed("Error")
        .setDescription("Nickname must contain [IGN] parameter");
    }

    if (nickname.contains("[GUILD_RANK]")) {
      List<GuildRole> guildRoleSettings = database.getAllGuildRoles(
        event.getGuild().getId()
      );
      guildRoleSettings.removeIf(
        o1 -> o1 == null || !o1.getEnableGuildRanks().equalsIgnoreCase("true")
      );
    ...
    if (nickname.replace("[IGN]", "").length() > 15) {
      return defaultEmbed("Error")
        .setDescription(
          "Nickname prefix and/or postfix must be less than or equal to 15 letters"
        );
    }
  ...
  private int updateVerifySettings(String key, String newValue) {
    JsonObject newVerifySettings = database
      .getVerifySettings(event.getGuild().getId())
      .getAsJsonObject();
    newVerifySettings.remove(key);
    newVerifySettings.addProperty(key, newValue);
    return database.updateVerifySettings(
      event.getGuild().getId(),
      newVerifySettings
    );
  }
  ...
  private EmbedBuilder getCurrentApplySettings(JsonElement applySettings) {
    ...
    eb.setDescription(
      "**" +
      displaySettings(applySettings, "enable").replace("•", "").trim() +
      "**"
    );
    eb.addField(
      "React Message Channel",
    ...
    eb.addField(
      "Waiting For Invite Channel",
      displaySettings(applySettings, "waitingChannelId"),
      true
    );
    eb.addField(
      "Staff Ping Role",
      displaySettings(applySettings, "staffPingRoleId"),
      true
    );
    eb.addField(
      "New Channel Prefix",
    ...
    eb.addField(
      "Ironman only",
      displaySettings(applySettings, "ironmanOnly"),
      true
    );
    eb.addField(
      "React Message Text",
    ...
    eb.addField(
      "Denied Message",
      displaySettings(applySettings, "denyMessageText"),
      true
    );
    eb.addField(
      "Requirements",
      displaySettings(applySettings, "applyReqs"),
      true
    );
    return eb;
  }
  ...
  private EmbedBuilder setWaitingChannel(String name, String textChannel) {
    try {
      TextChannel applyMessageTextChannel = event
        .getGuild()
        .getTextChannelById(textChannel.replaceAll("[<#>]", ""));
      ...
      eb.setDescription(
        ...
        applyMessageTextChannel.getAsMention()
      );
      return eb;
    } catch (Exception ignored) {}
    return defaultEmbed("Invalid Text Channel", null);
  ...
  private EmbedBuilder createApplyGuild(String name) {
    ...
    List<AutomatedApplication> currentApplys = database.getAllApplySettings(
      event.getGuild().getId()
    );
    ...
    int responseCode = database.updateApplySettings(
      event.getGuild().getId(),
      newApply
    );
    if (responseCode != 200) {
  ...
  private boolean allowApplyEnable(String name) {
    JsonObject currentSettings = database
      .getApplySettings(event.getGuild().getId(), name)
      .getAsJsonObject();
    currentSettings.remove("previousMessageId");
  ...
  private EmbedBuilder setIsIronman(String name, String isIronman) {
    if (
      isIronman.equalsIgnoreCase("true") || isIronman.equalsIgnoreCase("false")
    ) {
      int responseCode = updateApplySettings(
        name,
        "ironmanOnly",
        isIronman.toLowerCase()
      );
      if (responseCode != 200) {
        return defaultEmbed("Error")
          .setDescription("API returned response code " + responseCode);
      ...
      eb.setDescription(
        "**Ironman only:** " +
        (isIronman.equalsIgnoreCase("true") ? "enabled" : "disabled")
      );
      return eb;
    }
    return defaultEmbed("Invalid Input");
  ...
  private EmbedBuilder setApplyMessageTextChannelId(
    String name,
    String textChannel
  ) {
    try {
  ...
  private EmbedBuilder setApplyMessageStaffChannelId(
    String name,
    String textChannel
  ) {
    try {
      TextChannel staffTextChannel = event
        .getGuild()
        .getTextChannelById(textChannel.replaceAll("[<#>]", ""));
      int responseCode = updateApplySettings(
        name,
        "messageStaffChannelId",
        staffTextChannel.getId()
      );
      if (responseCode != 200) {
        return defaultEmbed("Error", null)
          .setDescription("API returned response code " + responseCode);
      ...
      eb.setDescription(
        "**Apply staff channel set to:** " + staffTextChannel.getAsMention()
      );
      return eb;
    } catch (Exception ignored) {}
    return defaultEmbed("Invalid Text Channel", null);
  ...
  private EmbedBuilder setApplyNewChannelPrefix(
    String name,
    ...
    if (channelPrefix.length() > 0) {
      if (EmojiParser.parseToAliases(channelPrefix).length() > 25) {
        return defaultEmbed("Error", null)
          .setDescription("Prefix cannot be longer than 25 letters!");
      }
      int responseCode = updateApplySettings(
        name,
        "newChannelPrefix",
        EmojiParser.parseToAliases(channelPrefix)
      );
      if (responseCode != 200) {
        return defaultEmbed("Error", null)
          .setDescription("API returned response code " + responseCode);
      ...
      eb.setDescription(
        "**Apply new channel prefix set to:** " + channelPrefix
      );
      return eb;
    }
    return defaultEmbed("Invalid Input", null);
  ...
  private EmbedBuilder setApplyAcceptMessageText(
    String name,
    String verifyText
  ) {
    if (verifyText.length() > 0) {
  ...
  private EmbedBuilder setApplyWaitListMessageText(
    String name,
    String verifyText
  ) {
    if (verifyText.length() > 0) {
  ...
  private EmbedBuilder setApplyDenyMessageText(String name, String denyText) {
    if (denyText.length() > 0) {
      ...
      int responseCode = updateApplySettings(
        name,
        "denyMessageText",
        EmojiParser.parseToAliases(denyText)
      );
      if (responseCode != 200) {
        return defaultEmbed("Error", null)
          .setDescription("API returned response code " + responseCode);
  ...
  private EmbedBuilder setApplyStaffPingRoleId(
    String name,
    String staffPingRoleMention
  ) {
    try {
      Role verifyGuildRole = event
        .getGuild()
        .getRoleById(staffPingRoleMention.replaceAll("[<@&>]", ""));
      if (!(verifyGuildRole.isPublicRole() || verifyGuildRole.isManaged())) {
        int responseCode = updateApplySettings(
          name,
          "staffPingRoleId",
          verifyGuildRole.getId()
        );
        if (responseCode != 200) {
          return defaultEmbed("Error", null)
            .setDescription("API returned response code " + responseCode);
        ...
        eb.setDescription(
          "**Apply staff ping role set to:** " + verifyGuildRole.getAsMention()
        );
        return eb;
      }
    } catch (Exception ignored) {}
  ...
  private EmbedBuilder setApplyNewChannelCategory(
    String name,
    String messageCategory
  ) {
    try {
      net.dv8tion.jda.api.entities.Category applyCategory = event
        .getGuild()
      ...
      eb.setDescription(
        "**Apply new channel category set to:** <#" +
        applyCategory.getId() +
        ">"
      );
      return eb;
  ...
  private EmbedBuilder removeApplyRequirement(String name, String reqNumber) {
    ...
    try {
      currentReqs =
        database.getApplyReqs(event.getGuild().getId(), name).getAsJsonArray();
    } catch (Exception ignored) {
      return defaultEmbed("Error")
        .setDescription("Unable to get current settings");
    }
    ...
    try {
      ...
      int responseCode = database.updateApplyReqs(
        event.getGuild().getId(),
        name,
        currentReqs
      );
  ...
  private EmbedBuilder addApplyRequirement(String name, String reqArgs) {
    ...
    if (currentReqs.size() >= 3) {
      return defaultEmbed("Error")
        .setDescription("You can only have up to 3 requirements");
    }
  ...
  private int updateApplySettings(String name, String key, String newValue) {
    JsonObject newApplyJson = database
      .getApplySettings(event.getGuild().getId(), name)
      .getAsJsonObject();
  ...
  private String displaySettings(JsonElement jsonSettings, String settingName) {
    if (higherDepth(jsonSettings, settingName) != null) {
      if (settingName.equals("applyReqs")) {
        JsonArray reqs = higherDepth(jsonSettings, settingName)
          .getAsJsonArray();
        ...
        JsonArray roles = higherDepth(jsonSettings, settingName)
          .getAsJsonArray();
        String ebStr = "";
      ...
      String currentSettingValue = higherDepth(jsonSettings, settingName)
        .getAsString();
        ...
        switch (settingName) {
          ...
          case "newChannelCategory":
            try {
              return (
                "<#" +
                event.getGuild().getCategoryById(currentSettingValue).getId() +
                ">"
              );
            } catch (PermissionException e) {
              if (e.getMessage().contains("Missing permission")) {
            ...
            break;
          case "enable":
            return currentSettingValue.equals("true")
              ? "• Enabled"
              : "• Disabled";
          case "guildId":
            try {
              ...
              return (
                "Error finding guild associated with " +
                currentSettingValue +
                " id"
              );
            }
