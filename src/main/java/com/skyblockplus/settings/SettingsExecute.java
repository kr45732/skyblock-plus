...
public class SettingsExecute {
        ...
        for (int i = 0; i < currentBlacklist.size(); i++) {
            if (
                ...
                return defaultSettingsEmbed()
                        .setDescription("Removed " + nameMcHyperLink(uuidStruct.username(), uuidStruct.uuid()) + " from the blacklist");
            }
        }

        return invalidEmbed(nameMcHyperLink(uuidStruct.username(), uuidStruct.uuid()) + " is not blacklisted");
    }
    ...
    public EmbedBuilder addApplyBlacklist(String username, String reason) {
        ...
        if (blacklistedUser != null) {
            return invalidEmbed(
                    nameMcHyperLink(uuidStruct.username(), uuidStruct.uuid()) +
        ...
        return defaultSettingsEmbed()
                .setDescription(
                        "Blacklisted " + nameMcHyperLink(uuidStruct.username(), uuidStruct.uuid()) + " with reason `" + reason + "`"
                );
    }
    ...
    public CustomPaginator.Builder getRolesSettings(JsonElement rolesSettings) {
        ...
        for (String roleName : roleNames) {
            ...
            switch (roleName) {
                ...
                case "alchemy", "combat", "fishing", "farming", "foraging", "carpentry", "mining", "taming", "enchanting", "skill_average", "pet_score", "catacombs" -> ebFieldString
                        .append("**A player's ")
                        .append(roleName)
                        .append(" level**\nExample: `")
                        .append(guildPrefix)
