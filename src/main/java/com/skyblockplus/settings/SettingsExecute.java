...
public class SettingsExecute {
    ...
    public void execute(CommandEvent event) {
        executor.submit(
                () -> {
                    ...
                    eb = null;
                    guildPrefix = getGuildPrefix(event.getGuild().getId());
                    ...
                    if (content.split(" ", 4).length == 4 && args[1].equals("set")) {
                        ...
                        eb.addField(
                                "Verify Settings",
                                "Use `" + guildPrefix + "settings verify` to see the current settings",
                                false
                        );
                        eb.addField(
                                "Apply Settings",
                                "Use `" + guildPrefix + "settings apply` to see the current settings",
                                false
                        );
                        eb.addField(
                                "Roles Settings",
                                "Use `" + guildPrefix + "settings roles` to see the current settings",
                                false
                        );
                        eb.addField(
                                "Guild Role/Ranks Settings",
                                "Use `" + guildPrefix + "settings guild` to see the current settings",
                                false
                        );
                        eb.addField(
                                "Mee6 Roles Settings",
                                "Use `" + guildPrefix + "settings mee6` to see the current settings",
                                false
                        );
    ...
    private EmbedBuilder createApplyGuild(String name) {
        ...
        List<AutomatedApply> currentApplications = database.getAllApplySettings(event.getGuild().getId());
