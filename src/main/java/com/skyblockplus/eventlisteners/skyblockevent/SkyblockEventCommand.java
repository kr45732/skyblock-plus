...
public class SkyblockEventCommand extends Command {
  ...
  public static void endSkyblockEvent(String guildId) {
    JsonElement runningEventSettings = database.getRunningEventSettings(
      guildId
    );
    JsonArray membersArr = higherDepth(runningEventSettings, "membersList")
      .getAsJsonArray();
    TextChannel announcementChannel = jda.getTextChannelById(
      higherDepth(runningEventSettings, "announcementId").getAsString()
    );
            ...
            try {
              return higherDepth(
                JsonParser.parseString(
                  uuidToUsernameResponse.getResponseBody()
                ),
                "username"
              )
                .getAsString();
    ...
    String eventType = higherDepth(runningEventSettings, "eventType")
      .getAsString();
    ...
    eventMemberList.sort(
      Comparator.comparingInt(o1 -> -Integer.parseInt(o1.getStartingAmount()))
    );
    ...
    try {
      ...
      ArrayList<String> prizeListKeys = getJsonKeys(
        higherDepth(runningEventSettings, "prizeMap")
      );
      for (int i = 0; i < prizeListKeys.size(); i++) {
        ebStringPrize
          ...
          .append(
            higherDepth(
              runningEventSettings,
              "prizeMap." + prizeListKeys.get(i)
            )
              .getAsString()
          )
          .append(" - ")
      ...
      if (ebStringPrize.length() > 0) {
        announcementChannel
          .sendMessage(
            defaultEmbed("Prizes")
              .setDescription(ebStringPrize.toString())
              .build()
          )
          .complete();
        database.updateSkyblockEventSettings(guildId, new SbEvent());
    ...
    announcementChannel
      .sendMessage(defaultEmbed("Prizes").setDescription("None").build())
      .complete();
    database.updateSkyblockEventSettings(guildId, new SbEvent());
  }
        ...
        if (args.length == 2 || args.length == 3) {
          switch (args[1]) {
            case "create":
              ...
              if (
                guildMap.containsKey(event.getGuild().getId()) &&
                ...
                ebMessage
                  .editMessage(
                    defaultEmbed("Error")
                      .setDescription("Event already running")
                      .build()
                  )
                  .queue();
              } else {
                ebMessage
                  .editMessage(
                    defaultEmbed("Error")
                      .setDescription("Cannot find server")
                      .build()
                  )
                  .queue();
              }
            ...
            case "current":
              if (database.getSkyblockEventActive(event.getGuild().getId())) {
                JsonElement currentSettings = database.getRunningEventSettings(
                ...
                eb.addField(
                  "Event Type",
                  capitalizeString(
                    higherDepth(currentSettings, "eventType").getAsString()
                  ),
                  false
                );
                ...
                eb.addField(
                  "End Date",
                  formatter.format(eventInstantEnding) + " UTC",
                  false
                );

                ArrayList<String> prizesKeys = getJsonKeys(
                  higherDepth(currentSettings, "prizeMap")
                );
                StringBuilder ebString = new StringBuilder();
                for (String prizePlace : prizesKeys) {
                  ebString
                    ...
                    .append(") - ")
                    .append(
                      higherDepth(currentSettings, "prizeMap." + prizePlace)
                        .getAsString()
                    )
                    .append("\n");
                }
                ...
                eb.addField(
                  "Members joined",
                  "" +
                  higherDepth(currentSettings, "membersList")
                    .getAsJsonArray()
                    .size(),
                  false
                );
                ...
                ebMessage
                  .editMessage(defaultEmbed("No event running").build())
                  .queue();
              }
              return;
            case "cancel":
              ...
              if (database.getSkyblockEventActive(event.getGuild().getId())) {
                ...
                if (code == 200) {
                  ebMessage
                    .editMessage(defaultEmbed("Event canceled").build())
                    .queue();
                } else {
                  ebMessage
                    .editMessage(
                      defaultEmbed("API returned code " + code).build()
                    )
                    .queue();
                }
              } else {
              ...
              if (database.getSkyblockEventActive(event.getGuild().getId())) {
                ...
                if (linkedAccount != null) {
                  ...
                  try {
                    ...
                    username =
                      higherDepth(linkedAccount, "minecraftUsername")
                        .getAsString();
                  } catch (Exception e) {
                    ebMessage
                      .editMessage(
                        defaultEmbed(
                          "You must be linked to run this command. Use `" +
                          BOT_PREFIX +
                          "link [IGN]` to link"
                        )
                          .build()
                  ...
                  try {
                    ...
                    ebMessage
                      .editMessage(
                        defaultEmbed("Error")
                          .setDescription(
                            "You must be in the guild to join the event"
                          )
                          .build()
                      )
                      .queue();
                  ...
                  if (
                    higherDepth(guildIn, "guild")
                      .getAsString()
                      .equals(
                        database.getSkyblockEventGuildId(
                          event.getGuild().getId()
                        )
                      )
                  ) {
                    Player player = args.length == 3
                      ? new Player(username, args[2])
                      : new Player(username);
                    ...
                    if (player.isValid()) {
                      ...
                      String eventType = higherDepth(
                        database.getRunningEventSettings(
                          event.getGuild().getId()
                        ),
                        "eventType"
                      )
                          ...
                          case "slayer":
                            {
                              ...
                              startingAmountFormatted =
                                startingAmount + " total slayer xp";
                              break;
                            }
                          case "skills":
                            {
                              ...
                              startingAmountFormatted =
                                startingAmount + " total skills xp";
                              break;
                            }
                          case "catacombs":
                            {
                              startingAmount =
                                player.getCatacombsSkill().totalSkillExp;
                              startingAmountFormatted =
                                startingAmount + " total catacombs xp";
                              break;
                            }
                        }
                        ...
                        if (startingAmount != -1) {
                          ...
                          if (code == 200) {
                            ...
                            ebMessage
                              .editMessage(
                                defaultEmbed("Error")
                                  .setDescription("API returned code " + code)
                                  .build()
                              )
                              .queue();
                          }
                      ...
                      ebMessage
                        .editMessage(
                          defaultEmbed("Error")
                            .setDescription("Unable to fetch player data")
                            .build()
                        )
                        .queue();
                    } else {
            ...
            case "leave":
              if (database.getSkyblockEventActive(event.getGuild().getId())) {
                ...
                if (linkedAccount != null) {
                  String uuid = higherDepth(linkedAccount, "minecraftUuid")
                    .getAsString();

                  ebMessage
                    .editMessage(
                      defaultEmbed("Success")
                        .setDescription("You left the event")
                        .build()
                    )
                    .queue();

                  database.removeEventMemberToRunningEvent(
                    event.getGuild().getId(),
                    uuid
                  );
                } else {
            ...
            case "lb":
              if (database.getSkyblockEventActive(event.getGuild().getId())) {
                if (!getGuildMap().containsKey(event.getGuild().getId())) {
                  ebMessage
                    .editMessage(defaultEmbed("No guild found").build())
                    .queue();
                  return;
                }

                AutomaticGuild currentGuild = getGuildMap()
                  .get(event.getGuild().getId());
                ...
                if (
                  ...
                  (
                    Duration
                      .between(
                        currentGuild.getEventMemberListLastUpdated(),
                        Instant.now()
                      )
                      .toMinutes() <
                    15
                  ...
                  for (int i = 0; i < eventMemberList.size(); i++) {
                    ...
                    ebString
                      ...
                      .append(
                        formatNumber(
                          Long.parseLong(eventMember.getStartingAmount())
                        )
                      )
                      .append("\n");
                  }
                ...
                for (JsonElement guildMember : membersArr) {
                  String guildMemberUuid = higherDepth(guildMember, "uuid")
                    .getAsString();
                  ...
                  try {
                    if (remainingLimit < 5) {
                      TimeUnit.SECONDS.sleep(timeTillReset);
                      System.out.println(
                        "Sleeping for " + timeTillReset + " seconds"
                      );
                    }
                  } catch (Exception ignored) {}
                  ...
                  asyncHttpClient
                    .prepareGet(
                      "https://api.ashcon.app/mojang/v2/user/" + guildMemberUuid
                    )
                    .execute()
                    .toCompletableFuture()
                    .thenApply(
                ...
                try {
                  if (!httpGetsFinishedLatch.await(20, TimeUnit.SECONDS)) {
                    ebMessage
                      .editMessage(defaultEmbed("Error fetching data").build())
                      .queue();
                    return;
                  }
                } catch (Exception e) {
                  System.out.println(
                    "== Stack Trace (Event Leaderboard Latch) =="
                  );
                  e.printStackTrace();
                }
                ...
                eventMemberList.sort(
                  Comparator.comparingInt(
                    o1 -> -Integer.parseInt(o1.getStartingAmount())
                  )
                );
                ...
                getGuildMap()
                  .get(event.getGuild().getId())
                  .setEventMemberList(eventMemberList);
                getGuildMap()
                  .get(event.getGuild().getId())
                  .setEventMemberListLastUpdated(Instant.now());
              } else {
                ebMessage
                  .editMessage(defaultEmbed("No event running").build())
                  .queue();
