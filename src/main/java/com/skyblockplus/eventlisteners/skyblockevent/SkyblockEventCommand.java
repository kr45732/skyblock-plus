    ...
    public static void endSkyblockEvent(String guildId) {
                                        ...
                                        .thenApply(
                                                guildMemberOuterProfileJsonResponse -> {
                                                    try {
                                                        ...
                                                        Player guildMemberPlayer = new Player(guildMemberUuid, guildMemberUsernameResponse, higherDepth(guildMember, "profileName").getAsString(), guildMemberOuterProfileJson, higherDepth(guildMember, "startingAmount").getAsLong());
        ...
        for (Player guildMember : guildMemberPlayersList) {
            try {
                switch (eventType) {
                    case "slayer": {
                        eventMemberList.add(new EventMember(guildMember.getUsername(), guildMember.getUuid(), "" + (guildMember.getSlayer() - guildMember.getStartingAmount()), guildMember.getProfileName()));
                        break;
                    }
                    case "skills": {
                        ...
                        eventMemberList.add(new EventMember(guildMember.getUsername(), guildMember.getUuid(), "" + (guildMember.getTotalSkillsXp() - guildMember.getStartingAmount()), guildMember.getProfileName()));
                        break;
                    }
                    case "catacombs": {
                        eventMemberList.add(new EventMember(guildMember.getUsername(), guildMember.getUuid(), "" + (guildMember.getCatacombsSkill().totalSkillExp - guildMember.getStartingAmount()), guildMember.getProfileName()));
                        break;
                    }
                }
            } catch (Exception ignored) {
        ...
        try {
            ...
            for (int i = 0; i < prizeListKeys.size(); i++) {
                ebStringPrize.append(i + 1).append(") ").append(higherDepth(higherDepth(runningEventSettings, "prizeMap"), prizeListKeys.get(i)).getAsString()).append(" - ").append(eventMemberList.get(i).getUsername()).append(" (").append(eventMemberList.get(i).getUuid()).append(")").append("\n");
            }
                ...
                switch (args[1]) {
                    ...
                    case "current":
                        if (database.getSkyblockEventActive(event.getGuild().getId())) {
                            ...
                            JsonElement guildJson = getJson("https://api.hypixel.net/guild?key=" + HYPIXEL_API_KEY + "&id=" + higherDepth(currentSettings, "eventGuildId").getAsString());
                            ...
                            for (String prizePlace : prizesKeys) {
                                ebString.append("• ").append(prizePlace).append(") - ").append(higherDepth(higherDepth(currentSettings, "prizeMap"), prizePlace).getAsString()).append("\n");
                            }
                    ...
                    case "cancel":
                        ...
                        if (database.getSkyblockEventActive(event.getGuild().getId())) {
                            int code = database.updateSkyblockEventSettings(event.getGuild().getId(), new SbEvent(new RunningEvent(), "false"));
                                ...
                                if (higherDepth(guildIn, "guild").getAsString().equals(database.getSkyblockEventGuildId(event.getGuild().getId()))) {
                                    ...
                                    if (player.isValid()) {
                                        ...
                                        try {
                                            ...
                                            if (startingAmount != -1) {
                                                int code = database.addEventMemberToRunningEvent(event.getGuild().getId(), new EventMember(username, uuid, "" + startingAmount, player.getProfileName()));

                                                if (code == 200) {
                                                    ebMessage.editMessage(defaultEmbed("Success").setDescription("You have joined the " + eventType + " Skyblock event as " + username + " on profile " + player.getProfileName() + " with a starting amount of " + startingAmountFormatted).build()).queue();
                                                } else {
                                                    ebMessage.editMessage(defaultEmbed("Error").setDescription("API returned code " + code).build()).queue();
                                                }
                                                return;
                                            }
                                        } catch (Exception ignored) {
                    ...
                    case "end":
                        ...
                        if (database.getSkyblockEventActive(event.getGuild().getId())) {
                            ...
                            ebMessage.editMessage(defaultEmbed("Success").setDescription("Event Ended").build()).queueAfter(3, TimeUnit.SECONDS);
                        } else {
                            ebMessage.editMessage(defaultEmbed("No event running").build()).queue();
                        }
                        return;
