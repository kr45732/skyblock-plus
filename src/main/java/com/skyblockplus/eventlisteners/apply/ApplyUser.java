...
public class ApplyUser implements Serializable {
  ...
  public ApplyUser(
    ...
    String channelPrefix = higherDepth(currentSettings, "newChannelPrefix")
      .getAsString();
    Category applyCategory = event
      .getGuild()
      .getCategoryById(
        higherDepth(currentSettings, "newChannelCategory").getAsString()
      );
    ...
    applicationChannel
      .sendMessage("Welcome " + applyingUser.getAsMention() + "!")
      .complete();
    ...
    EmbedBuilder welcomeEb = defaultEmbed(
      "Application for " + applyingUser.getName()
    );
  ...
  public boolean onMessageReactionAdd(MessageReactionAddEvent event) {
    ...
    TextChannel applicationChannel = jda.getTextChannelById(
      applicationChannelId
    );
    Message reactMessage = applicationChannel
      .retrieveMessageById(reactMessageId)
      .complete();
          ...
          event
            ...
            .contains(
              event
                .getGuild()
                .getRoleById(
                  higherDepth(currentSettings, "staffPingRoleId").getAsString()
                )
            ) ||
    ...
    if (event.getReactionEmote().getAsReactionCode().equals("❌")) {
      ...
      event.getReactionEmote().getAsReactionCode().equals("↩️") && state == 1
    ) {
      state = 2;
    } else if (
      !(
        (
          profileNameEmojis.contains(
            event.getReactionEmote().getAsReactionCode()
          ) &&
          (state == 0)
        ) ||
      ...
      reactMessage
        .clearReactions(event.getReactionEmote().getAsReactionCode())
        .queue();
      return false;
    }
    ...
    switch (state) {
      case 0:
        ...
        JsonArray currentReqs = higherDepth(currentSettings, "applyReqs")
          .getAsJsonArray();
        ...
        if (!meetReqs) {
          ...
          reactMessage =
            applicationChannel.sendMessage(reqEmbed.build()).complete();
          reactMessage.addReaction("✅").queue();
          this.reactMessageId = reactMessage.getId();
          state = 5;
          break;
        ...
        playerSkills =
          playerSkills.equals("-1") ? "API disabled" : playerSkills;

        try {
          playerCatacombs =
            roundAndFormat(
              player.getCatacombsSkill().skillLevel +
              player.getCatacombsSkill().progressToNext
            );
        } catch (Exception e) {
        ...
        ironmanSymbol =
          higherDepth(player.getOuterProfileJson(), "game_mode") != null
            ? " ♻️"
            : "";
        ...
        statsEmbed.addField(
          "Are the above stats correct?",
          "React with ✅ for yes, ↩️ to retry, and ❌ to cancel",
          false
        );

        reactMessage =
          applicationChannel.sendMessage(statsEmbed.build()).complete();
        reactMessage.addReaction("✅").queue();
      ...
      case 1:
        ...
        break;
      case 2:
        EmbedBuilder retryEmbed = defaultEmbed(
          "Application for " + applyingUser.getName()
        );
        ...
        reactMessage =
          applicationChannel.sendMessage(retryEmbed.build()).complete();
        this.reactMessageId = reactMessage.getId();
  ...
  private boolean onMessageReactionAddStaff(MessageReactionAddEvent event) {
    ...
    try {
      if (
        shouldDeleteChannel && (event.getMessageId().equals(reactMessageId))
      ) {
        if (event.getReactionEmote().getName().equals("✅")) {
          ...
          applicationChannel
            .delete()
            .reason("Applicant read final message")
    ...
    if (event.getReactionEmote().getEmoji().equals("❌")) {
      ...
      eb.setDescription(
        higherDepth(currentSettings, "denyMessageText").getAsString() +
        "\n**React with ✅ to close the channel**"
      );
      ...
      eb.setDescription(
        higherDepth(currentSettings, "acceptMessageText").getAsString() +
        "\n**React with ✅ to close the channel**"
      );
      ...
      try {
        TextChannel waitInviteChannel = jda.getTextChannelById(
      ...
      if (
        ...
        higherDepth(currentSettings, "waitlistedMessageText")
          .getAsString()
          .length() >
        0 &&
        !higherDepth(currentSettings, "waitlistedMessageText")
  ...
  private void staffCaseConstructor() {
    ...
    applyPlayerStats.setDescription(
      "**Total Skyblock weight:** " + playerWeight
    );
    applyPlayerStats.addField("Total slayer", playerSlayer, true);
    applyPlayerStats.addField(
      "Progress average skill level",
      playerSkills,
      true
    );
    ...
    JsonElement waitlistMsg = higherDepth(
      currentSettings,
      "waitlistedMessageText"
    );
    if (
      waitlistMsg != null &&
      waitlistMsg.getAsString().length() > 0 &&
      !waitlistMsg.getAsString().equals("none")
    ) {
      applyPlayerStats.addField(
        "To waitlist the application,",
        "React with \uD83D\uDD50",
        true
      );
    }
    ...
    staffChannel
      ...
      .complete();
    Message reactMessage = staffChannel
      .sendMessage(applyPlayerStats.build())
      .complete();
    reactMessage.addReaction("✅").queue();
    if (
      waitlistMsg != null &&
