...
public class ApplyUser implements Serializable {
    ...
    public ApplyUser(MessageReactionAddEvent event, JsonElement currentSettings, String playerUsername) {
        ...
        welcomeEb.setDescription("Please react with the emoji that corresponds to the profile you want to apply with or react with ❌ to cancel the application");
    ...
    public boolean onMessageReactionAdd(MessageReactionAddEvent event) {
        ...
        if (!event.getUser().equals(applyingUser)) {
            if (!(event.getGuild().getMember(event.getUser()).getRoles().contains(event.getGuild().getRoleById(higherDepth(currentSettings, "staffPingRoleId").getAsString())) || event.getGuild().getMember(event.getUser()).hasPermission(Permission.ADMINISTRATOR))) {
                reactMessage.removeReaction(event.getReactionEmote().getAsReactionCode(), event.getUser())
                        .queue();
                return false;
            }
        }
        ...
        switch (state) {
            ...
            case 2:
                ...
                retryEmbed.setDescription("Please react with the emoji that corresponds to the profile you want to apply with or react with ❌ to cancel the application");
...
//    private String playerProfileName;
//
//    public ApplyUser(MessageReactionAddEvent event, JsonElement currentSettings) {
//        User applyingUser = event.getUser();
//
//        logCommand(event.getGuild(), applyingUser, "apply " + applyingUser.getName());
//
