...
public class BidsCommand extends Command {
  ...
  private EmbedBuilder getPlayerBids(String username) {
    ...
    if (bids == null || bids.size() == 0) {
      return defaultEmbed(
        "No bids found for " + usernameUuidStruct.playerUsername
      );
    }
    ...
    for (JsonElement bid : bids) {
      String auctionDesc;
      String itemName;
      boolean isPet = higherDepth(bid, "item_lore")
        .getAsString()
        .toLowerCase()
        .contains("pet");

      Instant endingAt = Instant.ofEpochMilli(
        higherDepth(bid, "end").getAsLong()
      );
      ...
      if (
        higherDepth(bid, "item_name").getAsString().equals("Enchanted Book")
      ) {
        itemName =
          parseMcCodes(
            higherDepth(bid, "item_lore").getAsString().split("\n")[0]
          );
      } else {
        itemName =
          (
            isPet
              ? capitalizeString(
                higherDepth(bid, "tier").getAsString().toLowerCase()
              ) +
              " "
              : ""
          ) +
      ...
      if (timeUntil.length() > 0) {
        ...
        auctionDesc +=
          "\nHighest bidder: " +
          uuidToUsername(
            higherDepth(bidsArr.get(bidsArr.size() - 1), "bidder").getAsString()
          );
        for (int i = bidsArr.size() - 1; i >= 0; i--) {
          JsonElement curBid = bidsArr.get(i);
          if (
            higherDepth(curBid, "bidder")
              .getAsString()
              .equals(usernameUuidStruct.playerUuid)
          ) {
            auctionDesc +=
              "\nYour highest bid: " +
        ...
        auctionDesc =
          "Auction sold for " + simplifyNumber(highestBid) + " coins";
        auctionDesc +=
          "\n " +
          uuidToUsername(
            higherDepth(bidsArr.get(bidsArr.size() - 1), "bidder").getAsString()
  ...
  private JsonArray queryAhApi(String uuid) {
    ...
    try {
      ...
      URI uri = new URIBuilder(httpget.getURI())
        .addParameter("query", "{\"bids.bidder\":\"" + uuid + "\"}")
        .build();
      httpget.setURI(uri);
