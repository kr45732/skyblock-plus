...
public class BazaarCommand extends Command {
  ...
  private EmbedBuilder getBazaarItem(String itemName) {
    JsonElement bazaarItems = getJson(
      "https://api.slothpixel.me/api/skyblock/bazaar"
    );
    ...
    for (String item : itemsUnformated) {
      ...
      try {
        itemNameFormatted =
          higherDepth(bazaarItems, item + ".name").getAsString();
      } catch (Exception e) {
      ...
      if (
        itemNameFormatted
          .toUpperCase()
          .replace(" ", "_")
          .equals(formattedItemName)
      ) {
        JsonElement itemInfo = higherDepth(bazaarItems, item);
        EmbedBuilder eb = defaultEmbed(
          itemNameFormatted,
          "https://bazaartracker.com/product/" + item
        );
        eb.addField(
          "Buy Price (Per)",
          simplifyNumber(
            higherDepth(
              higherDepth(itemInfo, "buy_summary").getAsJsonArray().get(0),
              "pricePerUnit"
            )
              .getAsDouble()
          ),
        ...
        eb.addField(
          ...
          simplifyNumber(
            higherDepth(
              higherDepth(itemInfo, "sell_summary").getAsJsonArray().get(0),
              "pricePerUnit"
            )
              .getAsDouble()
          ),
        ...
        eb.addField(
          "Buy Volume",
          simplifyNumber(
            higherDepth(itemInfo, "quick_status.buyVolume").getAsDouble()
          ),
          true
        );
        eb.addField(
          "Sell Volume",
          simplifyNumber(
            higherDepth(itemInfo, "quick_status.sellVolume").getAsDouble()
          ),
          true
        );
        eb.addBlankField(true);
    ...
    if (
      closestMatch != null && higherDepth(bazaarItems, closestMatch) != null
    ) {
