    ...
    new Thread(
      () -> {
        ...
        if (args.split(" ").length >= 2) {
          ebMessage
            .editMessage(
              getLowestBin(args.replace(BOT_PREFIX + "bin ", "")).build()
            )
            .queue();
          return;
        }
  ...
  private EmbedBuilder getLowestBin(String item) {
    ...
    if (higherDepth(lowestBinJson, item) != null) {
      ...
      eb.addField(
        ...
        formatNumber(higherDepth(lowestBinJson, item).getAsLong()),
        false
      );
    ...
    JsonElement enchantsJson = higherDepth(
      getEnchantsJson(),
      "enchants_min_level"
    );
    ...
    for (String i : enchantNames) {
      if (preFormattedItem.contains(i)) {
        String enchantName;
        try {
          int enchantLevel = Integer.parseInt(
            preFormattedItem.replaceAll("\\D+", "")
          );
          ...
          try {
            ...
            for (int j = 10; j > 0; j--) {
              try {
                ...
                eb.addField(
                  capitalizeString(enchantName),
                  formatNumber(
                    higherDepth(lowestBinJson, formattedName).getAsLong()
                  ),
                  false
                );
            ...
            if (eb.getFields().size() == 0) {
              return defaultEmbed(
                "No bin found for " + capitalizeString(item.toLowerCase())
              );
            }
            eb.setThumbnail("https://sky.lea.moe/item.gif/ENCHANTED_BOOK");
            return eb;
      ...
      if (preFormattedItem.contains(i)) {
        ...
        for (Entry<String, String> j : rarityMap.entrySet()) {
          if (preFormattedItem.contains(j.getKey())) {
            petName =
              j.getKey().toLowerCase() +
              " " +
              formattedName.toLowerCase().replace("_", " ");
        ...
        if (!raritySpecified) {
          ...
          for (String j : petRarities) {
            if (
              higherDepth(lowestBinJson, formattedName + rarityMap.get(j)) !=
              null
            ) {
              petName =
                j.toLowerCase() +
    ...
    if (
      closestMatch != null && higherDepth(lowestBinJson, closestMatch) != null
    ) {
      EmbedBuilder eb = defaultEmbed("Lowest bin");
      if (enchantNames.contains(closestMatch.split(";")[0].trim())) {
        ...
        eb.addField(
          capitalizeString(
            closestMatch.toLowerCase().replace("_", " ").replace(";", " ")
          ),
        ...
        eb.addField(
          capitalizeString(
            rarityMapRev.get(itemS[1].toUpperCase()) +
            " " +
            itemS[0].replace("_", " ")
          ),
          formatNumber(higherDepth(lowestBinJson, closestMatch).getAsLong()),
