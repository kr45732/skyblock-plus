...
public class QueryAuctionCommand extends Command {
  ...
  private static JsonArray queryAhApi(String query) {
    ...
    try {
      ...
      try {
        ...
        System.out.println(
          "== Stack Trace (Auction Query Close Http Client) =="
        );
        e.printStackTrace();
      }
    }
  ...
  private EmbedBuilder queryAuctions(String query) {
    List<String> enchantsList = getJsonKeys(
      higherDepth(getEnchantsJson(), "enchants_min_level")
    );
      ...
      if (query.replace(" ", "_").toLowerCase().contains(enchant)) {
        ...
        if (ahQueryArr == null) {
          return defaultEmbed("Error")
            .setDescription("Error fetching auctions data");
        }

        EmbedBuilder eb = defaultEmbed("Query Auctions");

        for (JsonElement lowestBinAh : ahQueryArr) {
          Instant endingAt = Instant.ofEpochMilli(
            higherDepth(lowestBinAh, "end").getAsLong()
          );
          ...
          try {
            NBTCompound nbtData = NBTReader.readBase64(
              higherDepth(lowestBinAh, "item_bytes").getAsString()
            );
            ...
            if (itemStruct.getId().equals("ENCHANTED_BOOK")) {
              if (itemStruct.getEnchantsFormatted().contains(enchantName)) {
                String lowestBinStr = "";
                lowestBinStr +=
                  "**Name:** " + enchantName.replaceAll("[_;]", " ");
                lowestBinStr +=
                  "\n**Price:** " +
                  simplifyNumber(
                    higherDepth(lowestBinAh, "starting_bid").getAsDouble()
                  );
                lowestBinStr +=
                  "\n**Seller:** " +
                  uuidToUsername(
                    higherDepth(lowestBinAh, "auctioneer").getAsString()
                  );
                lowestBinStr +=
                  "\n**Auction:** `/ah " +
                  higherDepth(lowestBinAh, "uuid").getAsString() +
                  "`";
        ...
        return eb.setDescription(
          "No lowest bin found for `" + query + "` enchant"
        );
      }
    }
    ...
    for (String pet : petsList) {
      if (query.replace(" ", "_").toUpperCase().contains(pet)) {
        ...
        for (JsonElement lowestBinAh : ahQueryArr) {
          ...
          if (
            !rarity.equals("ANY") &&
            !higherDepth(lowestBinAh, "tier").getAsString().equals(rarity)
          ) {
            continue;
          }
          ...
          try {
            ...
            if (itemStruct.getId().equals("PET")) {
              ...
              lowestBinStr +=
                "**Name:** " +
                capitalizeString(
                  higherDepth(lowestBinAh, "tier").getAsString()
                ) +
                " " +
    ...
    EmbedBuilder eb = defaultEmbed("Query Auctions")
      .setDescription(
        "Found `" + ahQueryArr.size() + "` bins matching `" + query + "`"
      );
    ...
    for (JsonElement lowestBinAh : ahQueryArr) {
      ...
      String lowestBinStr = "";
      lowestBinStr +=
        "**Name:** " + higherDepth(lowestBinAh, "item_name").getAsString();
      lowestBinStr +=
        "\n**Price:** " +
