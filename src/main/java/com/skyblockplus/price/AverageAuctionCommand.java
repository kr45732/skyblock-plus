...
public class AverageAuctionCommand extends Command {
    ...
    new Thread(
      () -> {
        ...
        if (args.split(" ").length >= 2) {
          ebMessage
            .editMessage(getAverageAuctionPrice(args.split(" ", 2)[1]).build())
            .queue();
          return;
        }
  ...
  private EmbedBuilder getAverageAuctionPrice(String item) {
    JsonElement averageAhJson = getJson(
          ...
          try {
            ...
            for (int j = 10; j > 0; j--) {
              try {
                ...
                JsonElement itemJson = higherDepth(
                  averageAhJson,
                  formattedName
                );
            ...
            if (eb.getFields().size() == 0) {
              return defaultEmbed(
                "No auctions found for " + capitalizeString(item.toLowerCase())
              );
            }
            eb.setThumbnail("https://sky.lea.moe/item.gif/ENCHANTED_BOOK");
            return eb;
      ...
      if (internalName.contains(i)) {
        ...
        if (!raritySpecified) {
          ...
          for (String j : petRarities) {
            if (
              higherDepth(averageAhJson, formattedName + rarityMap.get(j)) !=
              null
            ) {
              petName =
                j.toLowerCase() +
    ...
    if (
      ...
      if (enchantNames.contains(closestMatch.split(";")[0].trim())) {
        ...
        if (higherDepth(itemJson, "clean_price") != null) {
          ...
          eb.addField(
            capitalizeString(
              rarityMapRev.get(itemS[1].toUpperCase()) + " " + itemS[0]
            ),
            formatNumber(higherDepth(itemJson, "clean_price").getAsLong()),
            false
          );
