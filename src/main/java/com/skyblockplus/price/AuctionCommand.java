...
public class AuctionCommand extends Command {
	...
	public static EmbedBuilder getPlayerAuction(String username, AuctionFilterType filterType, AuctionSortType sortType,
			boolean verbose, PaginatorEvent event) {
		...
		if (sortType == AuctionSortType.LOW || sortType == AuctionSortType.HIGH) {
			stream = stream.sorted(Comparator.comparingLong(auction -> (sortType == AuctionSortType.LOW ? 1 : -1) * Math
					.max(higherDepth(auction, "highest_bid_amount", 0L), higherDepth(auction, "starting_bid", 0))));
		}
		...
		if (!verbose) {
			...
			extras.setEveryPageTitle(usernameUuidStruct.getUsername())
					...
					.setEveryPageText((totalSoldValue > 0
							? "**Sold Auctions Value:** " + simplifyNumber(totalSoldValue)
									+ (auctionTax > 0
				...
				if (!higherDepth(currentAuction, "claimed").getAsBoolean()) {
					...
					if (duration.toMillis() > 0) {
						...
						if (highestBid >= startingBid) {
							...
							totalSoldValue += highestBid;
							aucTax = (highestBid > 1000000) ? ((0.99 * highestBid < 1000000) ? (highestBid - 1000000)
									: (long) (0.01 * highestBid)) : 0;
							auctionTax += aucTax;
							eb.addField("Status", "Sold for " + formatNumber(highestBid)
									+ (aucTax > 0
											? " - " + formatNumber(aucTax) + " = " + formatNumber(highestBid - aucTax)
											: "")
									+ " coins", false);
						} else {
					...
					try {
						double calculatedPrice = calc.calculateItemPrice(getGenericInventoryMap(
			...
			for (int i = 0; i < extras.getEmbedPages().size(); i++) {
				extras.getEmbedPages().set(i,
						extras.getEmbedPages().get(i)
								.setTitle(usernameUuidStruct.getUsername(),
										skyblockStatsLink(usernameUuidStruct.getUsername(), null))
								.setThumbnail(
										usernameUuidStruct.getAvatarlUrl())
								.setDescription((totalSoldValue > 0
										? "**Sold Auctions Value:** " + simplifyNumber(totalSoldValue)
												+ (auctionTax > 0
														? " - " + simplifyNumber(auctionTax) + " = "
																+ simplifyNumber(totalSoldValue - auctionTax)
	...
	public static EmbedBuilder getAuctionByUuid(String auctionUuid) {
		...
		String itemId = "None";
		try {
			itemId = NBTReader.readBase64(higherDepth(auctionJson, "item_bytes.data").getAsString()).getList("i")
					.getCompound(0).getString("tag.ExtraAttributes.id", "None");
		...
		if (duration.toMillis() > 0) {
			...
			if (highestBid >= startingBid) {
				ebStr += "\n**Auction sold** for " + simplifyNumber(highestBid) + " coins to "
						+ uuidToUsername(higherDepth(bidsArr.get(bidsArr.size() - 1), "bidder").getAsString())
								.getUsername();
			} else {
