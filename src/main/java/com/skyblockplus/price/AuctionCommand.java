...
public class AuctionCommand extends Command {
  ...
  private EmbedBuilder getPlayerAuction(String username, CommandEvent event) {
    ...
    JsonElement playerAuctions = getJson(
      "https://api.hypixel.net/skyblock/auction?key=" +
      HYPIXEL_API_KEY +
      "&player=" +
    ...
    for (int i = 0; i < auctionsArray.size(); i++) {
      ...
      if (!higherDepth(currentAuction, "claimed").getAsBoolean()) {
        String auction;
        boolean isPet = higherDepth(currentAuction, "item_lore")
          .getAsString()
          .toLowerCase()
          .contains("pet");
        boolean bin = false;
        ...
        Instant endingAt = Instant.ofEpochMilli(
          higherDepth(currentAuction, "end").getAsLong()
        );
        ...
        if (
          higherDepth(currentAuction, "item_name")
            .getAsString()
            .equals("Enchanted Book")
        ) {
          auctions[i][0] =
            parseMcCodes(
              higherDepth(currentAuction, "item_lore")
                .getAsString()
                .split("\n")[0]
            );
        } else {
          auctions[i][0] =
            (
              isPet
                ? capitalizeString(
                  higherDepth(currentAuction, "tier")
                    .getAsString()
                    .toLowerCase()
                ) +
                " "
        ...
        long highestBid = higherDepth(currentAuction, "highest_bid_amount")
          .getAsInt();
        long startingBid = higherDepth(currentAuction, "starting_bid")
          .getAsInt();
    ...
    eb.setTitle(
      "No auctions found for " + usernameUuidStruct.playerUsername,
      null
    );
    return eb;
  }
