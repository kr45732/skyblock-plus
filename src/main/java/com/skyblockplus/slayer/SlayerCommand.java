...
public class SlayerCommand extends Command {
  ...
  protected void execute(CommandEvent event) {
    new Thread(
      () -> {
        ...
        if (args.length == 3) {
          ebMessage
            .editMessage(getPlayerSlayer(args[1], args[2]).build())
            .queue();
          return;
  ...
  private EmbedBuilder getPlayerSlayer(String username, String profileName) {
    ...
    if (player.isValid()) {
      ...
      JsonElement slayer = higherDepth(
        player.getProfileJson(),
        "slayer_bosses"
      );
      ...
      int revTwoKills = higherDepth(slayer, "zombie.boss_kills_tier_1") != null
        ...
        : 0;
      int revThreeKills = higherDepth(slayer, "zombie.boss_kills_tier_2") !=
        null
      ...
      int taraTwoKills = higherDepth(slayer, "spider.boss_kills_tier_1") != null
        ...
        : 0;
      int taraThreeKills = higherDepth(slayer, "spider.boss_kills_tier_2") !=
        null
        ? higherDepth(slayer, "spider.boss_kills_tier_2").getAsInt()
        : 0;
      int taraFourKills = higherDepth(slayer, "spider.boss_kills_tier_3") !=
        null
        ? higherDepth(slayer, "spider.boss_kills_tier_3").getAsInt()
        : 0;

      int endermanOneKills = higherDepth(
          slayer,
          "enderman.boss_kills_tier_0"
        ) !=
        null
        ? higherDepth(slayer, "enderman.boss_kills_tier_0").getAsInt()
        : 0;
      int endermanTwoKills = higherDepth(
          slayer,
          "enderman.boss_kills_tier_1"
        ) !=
        null
        ? higherDepth(slayer, "enderman.boss_kills_tier_1").getAsInt()
        : 0;
      int endermanThreeKills = higherDepth(
          slayer,
          "enderman.boss_kills_tier_2"
        ) !=
        null
        ? higherDepth(slayer, "enderman.boss_kills_tier_2").getAsInt()
        : 0;
      int endermanFourKills = higherDepth(
          slayer,
          "enderman.boss_kills_tier_3"
        ) !=
        null
      ...
      eb.addField(
        "<:sven_packmaster:800002277648891914> Wolf (" +
        player.getSlayerLevel("sven") +
        ")",
        simplifyNumber(player.getWolfXp()) + " XP",
        true
      );
      eb.addField(
        "<:revenant_horror:800002290987302943> Zombie (" +
        player.getSlayerLevel("rev") +
        ")",
        simplifyNumber(player.getZombieXp()) + " XP",
        true
      );
      eb.addField(
        "<:tarantula_broodfather:800002277262884874> Spider (" +
        player.getSlayerLevel("tara") +
        ")",
      ...
      eb.addField(
        "<:voidgloom_seraph:849280131281059881> Enderman (" +
        player.getSlayerLevel("enderman") +
        ")",
