	...
	public static EmbedBuilder getCalcSlayer(String username, String profileName, String slayerType, int targetLevel, long targetXp) {
		...
		if (player.isValid()) {
			int curXp = player.getSlayer(slayerType);
			 targetXp = targetLevel != -1 ? higherDepth(
					getLevelingJson(),
					"slayer_xp." +
							switch (slayerType) {
								case "sven" -> "wolf";
			...
			for(int i=0; i<(slayerType.equals("enderman") ? 4 : 5); i++){
				...
				long cost = killsNeeded * switch (i){
					case 0 ->2000L;
					case 1 -> 7500L;
					case 2-> 20000L;
					case 3-> 50000L;
				...
				out.append("\n").append(SLAYER_EMOJI_MAP.get(slayerType)).append(" Tier ").append(toRomanNumerals(i + 1).toUpperCase()).append(" ").append(capitalizeString(slayerType)).append(": ").append(formatNumber(killsNeeded)).append(" ($").append(formatNumber(cost)).append(")");
			}
