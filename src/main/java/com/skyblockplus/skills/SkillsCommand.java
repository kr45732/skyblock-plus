  ...
  protected void execute(CommandEvent event) {
    new Thread(
      () -> {
        ...
        if (args.length == 3) {
          ebMessage
            .editMessage(getPlayerSkill(args[1], args[2]).build())
            .queue();
          return;
      ...
      for (String skill : skills) {
        ...
        if (skillInfo != null) {
          eb.addField(
            ...
            "\nProgress: " +
            (
              skillInfo.skillLevel == skillInfo.maxSkillLevel
                ? "MAX"
          ...
          eb.addField(
            skillsEmojiMap.get(skill) + " " + capitalizeString(skill) + " (?) ",
            "Unable to retrieve",
            true
          );
        }
