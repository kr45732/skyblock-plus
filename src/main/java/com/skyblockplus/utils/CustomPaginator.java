...
public class CustomPaginator extends Menu {
  ...
  CustomPaginator(
    ...
    this.pages =
      (int) Math.ceil(
        (double) (
          extras.getEmbedFields().size() > 0
            ? extras.getEmbedFields().size()
            : strings.size()
        ) /
        itemsPerPage
      );
    ...
    action.queue(
      m -> {
        if (pages > 1) {
          ...
          m
            .addReaction(bulkSkipNumber > 1 ? BIG_RIGHT : RIGHT)
    ...
    switch (event.getReaction().getReactionEmote().getName()) {
      ...
      case BIG_LEFT:
        if (newPageNum > 1 || wrapPageEnds) {
          for (
            int i = 1;
            (newPageNum > 1 || wrapPageEnds) && i < bulkSkipNumber;
            i++
          ) {
      ...
      case BIG_RIGHT:
        if (newPageNum < pages || wrapPageEnds) {
          for (
            int i = 1;
            (newPageNum < pages || wrapPageEnds) && i < bulkSkipNumber;
            i++
          ) {
  ...
  private Message renderPage(int pageNum) {
    ...
    if (showPageNumbers) {
      embedBuilder.setFooter(
        "Created By CrypticPlasma • Page " + pageNum + "/" + pages,
        null
      );
    }
  ...
  public static class Builder
    extends Menu.Builder<CustomPaginator.Builder, CustomPaginator> {
    ...
    public CustomPaginator build() {
      ...
      Checks.check(
        !strings.isEmpty() || !extras.getEmbedFields().isEmpty(),
    ...
    public Builder setColumns(int columns) {
      if (columns < 1 || columns > 3) throw new IllegalArgumentException(
        "Only 1, 2, or 3 columns are supported"
      );
      this.columns = columns;
      return this;
    }
