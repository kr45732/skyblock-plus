...
public class Utils {
  ...
  public static final ScriptEngine jsScriptEngine = new ScriptEngineManager()
    .getEngineByName("js");
  ...
  public static void setApplicationSettings() {
    ...
    try {
      ...
      String[] database_url_unformatted =
        ((String) appProps.get("DATABASE_URL")).split(":", 3);
      ...
      String[] database_url_unformatted = System
        .getenv("DATABASE_URL")
        .split(":", 3);
      DATABASE_USERNAME = database_url_unformatted[1].replace("/", "");
      DATABASE_PASSWORD = database_url_unformatted[2].split("@")[0];
  ...
  public static JsonElement getBitsJson() {
    if (bitsJson == null) {
      bitsJson =
        getJson(
          "https://raw.githubusercontent.com/plun1331/SkyKings/main/bot-data/bit-prices.json"
        );
    }
  ...
  public static JsonElement parseJsString(String jsString) {
    try {
      return JsonParser.parseString(
        jsScriptEngine
          .eval(String.format("JSON.stringify(%s);", jsString))
          .toString()
      );
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static String convertSkyblockIdName(String itemName) {
    ...
    try {
      JsonArray collectionsArray = higherDepth(
        collectionsJson,
        "collection_data"
      )
        .getAsJsonArray();
      for (JsonElement collection : collectionsArray) {
        try {
          if (
            higherDepth(collection, "skyblockId").getAsString().equals(itemName)
          ) {
    ...
    try {
      ...
      if (jsonUrl.toLowerCase().contains("api.hypixel.net")) {
        try {
          ...
          timeTillReset =
            Integer.parseInt(
              httpresponse.getFirstHeader("RateLimit-Reset").getValue()
            );
        } catch (Exception ignored) {}
      }
  ...
  public static EmbedBuilder errorMessage(String name) {
    return defaultEmbed(
      "Invalid input. Type `" + BOT_PREFIX + "help " + name + "` for help"
    );
  }
  ...
  public static String convertToInternalName(String itemName) {
    ...
    switch (preFormattedItem) {
      ...
      default:
        if (
          preFormattedItem.contains("GOLDEN") &&
          preFormattedItem.contains("HEAD")
        ) {
    ...
    if (botLogChannel == null) {
      botLogChannel =
        jda
          ...
          .getTextChannelById("818469899848515624");
    }
  ...
  public static DiscordInfoStruct getPlayerDiscordInfo(String username) {
    try {
      ...
      String discordTag = higherDepth(
        ...
        .getAsString();
      String minecraftUsername = higherDepth(playerJson, "player.displayname")
        .getAsString();
      String minecraftUuid = higherDepth(playerJson, "player.uuid")
        .getAsString();
  ...
  public static String parseMcCodes(String unformattedString) {
    return unformattedString.replaceAll(
      "§f|§a|§9|§5|§6|§d|§4|§c|§7|§8|§l|§o|§b|§2|§e|§r|§3|§1|§ka",
      ""
    );
  }

  public static CustomPaginator.Builder defaultPaginator(
    EventWaiter waiter,
    User eventAuthor
  ) {
