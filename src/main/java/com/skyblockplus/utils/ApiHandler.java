...
public class ApiHandler {
	...
	private static final Pattern minecraftUuidRegex = Pattern
			.compile("[0-9a-f]{32}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}");
	...
	public static UsernameUuidStruct usernameToUuid(String username) {
		...
		Map.Entry<String, String> cachedResponse = uuidToUsernameCache.asMap().entrySet().stream()
				.filter(entry -> entry.getValue().equalsIgnoreCase(username)).findFirst().orElse(null);
	...
	public static CompletableFuture<String> asyncUuidToUsername(String uuid) {
		...
		if (cachedResponse != null) {
			...
			future = asyncHttpClient
					.prepareGet((useAlternativeApi ? "https://playerdb.co/api/player/minecraft/"
					...
					.execute().toCompletableFuture().thenApply(uuidToUsernameResponse -> {
						try {
							String username = Utils
									.higherDepth(JsonParser.parseString(uuidToUsernameResponse.getResponseBody()),
	...
	public static CompletableFuture<JsonElement> asyncSkyblockProfilesFromUuid(String uuid, String hypixelApiKey) {
		...
		if (cachedResponse != null) {
			...
			future = asyncHttpClient
					...
					.execute().toCompletableFuture().thenApply(profilesResponse -> {
						try {
							try {
								keyCooldownMap.get(hypixelApiKey).getRemainingLimit()
										.set(Integer.parseInt(profilesResponse.getHeader("RateLimit-Remaining")));
								keyCooldownMap.get(hypixelApiKey).getTimeTillReset()
	...
	public static JsonArray getBidsFromPlayer(String uuid) {
		try {
			...
			URI uri = new URIBuilder(httpget.getURI()).addParameter("bids", "[{\"bidder\":\"" + uuid + "\"}]")
					.addParameter("limit", "-1").addParameter("key", AUCTION_API_KEY).build();
			httpget.setURI(uri);
	...
	public static JsonArray queryLowestBin(String query) {
		try {
			...
			URI uri = new URIBuilder(httpget.getURI()).addParameter("end", "" + Instant.now().toEpochMilli())
					.addParameter("item_name", "%" + query + "%").addParameter("sort", "ASC").addParameter("limit", "1")
					.addParameter("key", AUCTION_API_KEY).build();
			httpget.setURI(uri);
	...
	public static JsonArray queryLowestBinPet(String petName, String rarity) {
		try {
			...
			URIBuilder uri = new URIBuilder(httpGet.getURI()).addParameter("end", "" + Instant.now().toEpochMilli())
					.addParameter("item_name", "%" + petName + "%").addParameter("item_id", "PET")
					.addParameter("sort", "ASC").addParameter("limit", "1").addParameter("key", AUCTION_API_KEY);
	...
	public static JsonArray queryLowestBinEnchant(String enchantId, int enchantLevel) {
		try {
			...
			URI uri = new URIBuilder(httpGet.getURI()).addParameter("end", "" + Instant.now().toEpochMilli())
					...
					.addParameter("enchants", enchantId.toUpperCase() + ";" + enchantLevel).addParameter("sort", "ASC")
					.addParameter("limit", "1").addParameter("key", AUCTION_API_KEY).build();
			httpGet.setURI(uri);
	...
	public static void cacheJson(String playerUuid, JsonElement json) {
		executor.submit(() -> {
			try (Statement statement = getCacheDatabaseConnection().createStatement()) {
				...
				statement.executeUpdate("INSERT INTO profiles VALUES ('" + playerUuid + "', " + now.toEpochMilli()
						+ ", '" + json
	...
	public static void updateLinkedAccounts() {
		try {
			database.getLinkedUsers().stream().filter(linkedAccountModel -> Duration
					...
					.toDays() > 5).limit(10)
					.forEach(o -> asyncUuidToUsername(o.getMinecraftUuid()).thenApply(username -> {
						if (username != null) {
							database.addLinkedUser(new LinkedAccountModel("" + Instant.now().toEpochMilli(),
									o.getDiscordId(), o.getMinecraftUuid(), username));
						}
						return null;
					}));
		} catch (Exception e) {
