...
public class EmojiUpdater {
        ...
        try {
            ...
            for (JsonElement i : sbItems) {
                ...
                if (higherDepth(i, "skin") != null && allSbItems.contains(id)) {
                    out.addProperty(id, "https://sky.shiiyu.moe/head/" + higherDepth(JsonParser.parseString(new String(Base64.getDecoder().decode(higherDepth(i, "skin").getAsString()))), "textures.SKIN.url", "").split("://textures.minecraft.net/texture/")[1]);
            ...
            for (File file : dir.listFiles()) {
                if (file.isDirectory()) {
                    ...
                    if (file.getName().endsWith(".properties")) {
                        ...
                        if (textureFile == null) {
                            System.out.println("Texture: " +id + " " + file.getPath());
                            continue;
                        }
                        ...
                        if (getInternalJsonMappings().keySet().contains(sId)) {
                            ...
                            System.out.println("Invalid ID: " + sId + " " + file.getPath());
                        }
                    }
                }
            }
    ...
    public static void runEmojis(String parsedItemsUrl) {
        ...
        try {
            ...
            }).filter(g -> g.getEmotes().size() < g.getMaxEmotes()).sorted(Comparator.comparingInt(g -> Integer.parseInt(g.getName().split("Skyblock Plus - Emoji Server ")[1]))).collect(Collectors.toList());
            int guildCount = 0;
