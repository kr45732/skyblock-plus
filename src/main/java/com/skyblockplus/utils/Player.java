...
public class Player {
  ...
  public Player(
    ...
    JsonElement outerProfileJson,
    String playerGuildRank
  ) {
    ...
    try {
      if (
        getLatestProfile(
          higherDepth(outerProfileJson, "profiles").getAsJsonArray()
        )
      ) {
        return;
      }
  ...
  /* Inventory */
  public static Map<Integer, InvItem> getGenericInventoryMap(
    NBTCompound parsedContents
  ) {
    try {
      ...
      for (int i = 0; i < items.size(); i++) {
        try {
          ...
          if (!item.isEmpty()) {
            ...
            itemInfo.setName(
              parseMcCodes(item.getString("tag.display.Name", "None"))
            );
            itemInfo.setLore(
              parseMcCodes(
                item
                  .getString("tag.display.Lore", "None")
                  .replace(", ", "\n")
                  .replace("[", "")
                  .replace("]", "")
              )
            ...
            itemInfo.setCount(
              Integer.parseInt(item.getString("Count", "0").replace("b", " "))
            );
            itemInfo.setId(item.getString("tag.ExtraAttributes.id", "None"));
            itemInfo.setCreationTimestamp(
              item.getString("tag.ExtraAttributes.timestamp", "None")
            );
            itemInfo.setHbpCount(
              item.getInt("tag.ExtraAttributes.hot_potato_count", 0)
            );
            itemInfo.setRecombobulated(
              item.getInt("tag.ExtraAttributes.rarity_upgrades", 0) == 1
            );
            itemInfo.setModifier(
              item.getString("tag.ExtraAttributes.modifier", "None")
            );
            itemInfo.setDungeonFloor(
              Integer.parseInt(
                item.getString("tag.ExtraAttributes.item_tier", "0")
              )
            );
            itemInfo.setNbtTag(item.toString());

            try {
              NBTCompound enchants = item.getCompound(
                "tag.ExtraAttributes.enchantments"
              );
            ...
            String itemSkinStr = item.getString(
              "tag.ExtraAttributes.skin",
              "None"
            );
            ...
            try {
              NBTList necronBladeScrolls = item.getList(
                "tag.ExtraAttributes.ability_scroll"
              );
            ...
            if (
              item.getInt("tag.ExtraAttributes.wood_singularity_count", 0) == 1
            ) {
              itemInfo.addExtraValue("WOOD_SINGULARITY");
            }

            try {
              byte[] backpackContents = item.getByteArray(
                "tag.ExtraAttributes." +
                itemInfo.getId().toLowerCase() +
                "_data"
              );
              NBTCompound parsedContentsBackpack = NBTReader.read(
                new ByteArrayInputStream(backpackContents)
              );
              itemInfo.setBackpackItems(
                getGenericInventoryMap(parsedContentsBackpack).values()
              );
            } catch (Exception ignored) {}
  ...
  public String[] getAllProfileNames(boolean isIronman) {
    ...
    for (JsonElement profile : profilesArray) {
      try {
        ...
        profileNameList.add(
          higherDepth(profile, "cute_name").getAsString().toLowerCase()
        );
      } catch (Exception ignored) {}
    }
  ...
  public double getSkillAverage() {
    ...
    for (String skill : skills) {
      try {
        double skillExp = higherDepth(profileJson, "experience_skill_" + skill)
          .getAsDouble();
  ...
  public SkillsStruct skillInfoFromExp(double skillExp, String skill) {
    ...
    if (skill.equals("catacombs")) {
      skillsTable =
        higherDepth(getLevelingJson(), "catacombs").getAsJsonArray();
    } else if (skill.equals("runecrafting")) {
      skillsTable =
        higherDepth(getLevelingJson(), "runecrafting_xp").getAsJsonArray();
    } else {
      skillsTable =
        higherDepth(getLevelingJson(), "leveling_xp").getAsJsonArray();
    }
    int maxLevel;
    try {
    ...
    long xpForNext = 0;
    if (level < maxLevel) xpForNext =
      (long) Math.ceil(skillsTable.get(level).getAsLong());

    double progress = xpForNext > 0
      ? Math.max(0, Math.min(((double) xpCurrent) / xpForNext, 1))
      : 0;

    return new SkillsStruct(
      skill,
      level,
      maxLevel,
      (long) skillExp,
      ...
      for (int i = 0; i < profilesArray.size(); i++) {
        ...
        try {
          lastSaveLoop =
            higherDepth(
              profilesArray.get(i),
              "members." + this.playerUuid + ".last_save"
            )
              .getAsString();
        } catch (Exception e) {
          continue;
        ...
        if (i == 0) {
          this.profileJson =
            higherDepth(profilesArray.get(i), "members." + this.playerUuid);
          this.outerProfileJson = profilesArray.get(i);
          lastProfileSave = lastSaveLoop;
          this.profileName =
            higherDepth(profilesArray.get(i), "cute_name").getAsString();
        } else if (
          Instant
            .ofEpochMilli(Long.parseLong(lastSaveLoop))
  ...
  public boolean profileIdFromName(
    String profileName,
    JsonArray profilesArray
  ) {
    try {
      for (int i = 0; i < profilesArray.size(); i++) {
        String currentProfileName = higherDepth(
          profilesArray.get(i),
          "cute_name"
        )
          .getAsString();
  ...
  public int petLevelFromXp(long petExp, String rarity) {
    int petRarityOffset = higherDepth(
      getPetJson(),
      "pet_rarity_offset." + rarity.toUpperCase()
    )
      .getAsInt();
    JsonArray petLevelsXpPer = higherDepth(getPetJson(), "pet_levels")
      .getAsJsonArray();
    long totalExp = 0;
  ...
  public int getNumberMinionSlots() {
    try {
      List<String> profileMembers = getJsonKeys(
        higherDepth(outerProfileJson, "members")
      );
      ...
      for (String member : profileMembers) {
        try {
          JsonArray craftedMinions = higherDepth(
            outerProfileJson,
            "members." + member + ".crafted_generators"
          )
            .getAsJsonArray();
  ...
  public int getSlayerLevel(String slayerName) {
    switch (slayerName) {
      case "sven":
        JsonArray wolfLevelArray = higherDepth(
          getLevelingJson(),
          "slayer_xp.wolf"
        )
          .getAsJsonArray();
      ...
      case "rev":
        JsonArray zombieLevelArray = higherDepth(
          getLevelingJson(),
          "slayer_xp.zombie"
        )
          .getAsJsonArray();
      ...
      case "tara":
        JsonArray spiderLevelArray = higherDepth(
          getLevelingJson(),
          "slayer_xp.spider"
        )
          .getAsJsonArray();
      ...
      case "enderman":
        JsonArray endermanLevelArray = higherDepth(
          getLevelingJson(),
          "slayer_xp.spider"
        )
          .getAsJsonArray(); // TODO:
  ...
  public int getEndermanXp() {
    ...
    return higherDepth(profileSlayer, "enderman.xp") != null
      ? higherDepth(profileSlayer, "enderman.xp").getAsInt()
      : 0;
  }
  ...
  public Map<Integer, ArmorStruct> getWardrobeList() {
    try {
      String encodedWardrobeContents = higherDepth(
        profileJson,
        "wardrobe_contents.data"
      )
        .getAsString();
      int equippedSlot = higherDepth(profileJson, "wardrobe_equipped_slot")
        .getAsInt();
      NBTCompound decodedWardrobeContents = NBTReader.readBase64(
        encodedWardrobeContents
      );
      ...
      for (int i = 0; i < wardrobeFrames.size(); i++) {
        NBTCompound displayName = wardrobeFrames
          .getCompound(i)
          .getCompound("tag.display");
        if (displayName != null) {
          wardrobeFramesMap.put(
            i,
      ...
      for (int i = 0; i < 9; i++) {
        ...
        for (
          int j = (wardrobeFramesMap.size() / 2) + i;
          j < wardrobeFramesMap.size();
          j += 9
        ) {
          String currentArmorPiece = wardrobeFramesMap.get(j);
          if ((j - i) / 9 == 4) {
      ...
      if (equippedSlot > 0) {
        armorStructMap.replace(
          (equippedSlot - 1),
          getInventoryArmor().makeBold()
        );
      }
  ...
  public List<String[]> getWardrobe() {
    try {
      int equippedWardrobeSlot = higherDepth(
        profileJson,
        "wardrobe_equipped_slot"
      )
        .getAsInt();
      Map<Integer, InvItem> equippedArmor = equippedWardrobeSlot != -1
        ? getInventoryArmorMap()
        : null;

      String encodedInventoryContents = higherDepth(
        profileJson,
        "wardrobe_contents.data"
      )
        .getAsString();
      NBTCompound decodedInventoryContents = NBTReader.readBase64(
        encodedInventoryContents
      );
      ...
      for (int i = 0; i < invFrames.size(); i++) {
        NBTCompound displayName = invFrames
          .getCompound(i)
          .getCompound("tag.ExtraAttributes");
        ...
        if (displayName != null) {
          ...
          ((i + 1) <= 36) &&
          (equippedArmor.get((((i + 1) - equippedWardrobeSlot) / 9) + 1)) !=
          null
        ) {
          invFramesMap.put(
            i + 1,
            equippedArmor
              .get((((i + 1) - equippedWardrobeSlot) / 9) + 1)
              .getId()
              .toLowerCase()
          );
        } else if (
          ...
          ((((i + 1) - equippedWardrobeSlot) % 9) == 0) &&
          ((i + 1) > 36) &&
          (equippedArmor.get((((i + 1) - equippedWardrobeSlot) / 9) + 1 - 3)) !=
          null
        ) {
          invFramesMap.put(
            i + 1,
            equippedArmor
              .get((((i + 1) - equippedWardrobeSlot) / 9) + 1 - 3)
              .getId()
              .toLowerCase()
          );
        } else {
      ...
      for (Map.Entry<Integer, String> i : invFramesMap.entrySet()) {
        ...
        if (i.getKey() != 0 && i.getKey() % 36 == 0) {
          enderChestPages.add(
            new String[] {
              outputStringPart1.toString(),
              outputStringPart2.toString(),
            }
          );
  ...
  public ArmorStruct getInventoryArmor() {
    try {
      ...
      for (int i = 0; i < talismanFrames.size(); i++) {
        NBTCompound displayName = talismanFrames
          .getCompound(i)
          .getCompound("tag.display");
        if (displayName != null) {
          armorFramesMap.put(
      ...
      return new ArmorStruct(
        armorFramesMap.get(3),
        armorFramesMap.get(2),
        armorFramesMap.get(1),
  ...
  public String itemToEmoji(String itemName) {
    ...
    emojiMap.put(
      "overflux_power_orb",
      "<:overflux_power_orb:814676605514678292>"
    );
    ...
    emojiMap.put(
      "aspect_of_the_end",
      "<:aspect_of_the_end:814689179110735902>"
    );
    ...
    emojiMap.put(
      "dungeon_chest_key",
      "<:dungeon_chest_key:815284007767769100>"
    );
    emojiMap.put(
      "florid_zombie_sword",
      "<:florid_zombie_sword:815284420567236649>"
    );
    ...
    emojiMap.put(
      "unstable_fragment",
      "<:unstable_fragment:815285660264038400>"
    );
    ...
    emojiMap.put(
      ...
      "<:infinite_superboom_tnt:815305646194688000>"
    );
    ...
    emojiMap.put(
      ...
      "<:super_heavy_chestplate:815307825504976916>"
    );
    ...
    emojiMap.put(
      ...
      "<:storm_chestplate:816442344265744395>"
    );
    ...
    emojiMap.put(
      ...
      "<:skeleton_master_leggings:816442344387641385>"
    );
    emojiMap.put(
      "wise_wither_leggings",
      "<:storm_leggings:816442344459075584>"
    );
    emojiMap.put(
      "personal_compactor_7000",
      "<:personal_compactor_7000:816442344462090281>"
    );
    ...
    emojiMap.put(
      ...
      "<:zombie_soldier_boots:816442344617934868>"
    );
    emojiMap.put(
      "shadow_assassin_helmet",
      "<:shadow_assassin_helmet:816444761559793694>"
    );
    emojiMap.put("wither_catalyst", "<:wither_catalyst:816444761493864448>");
    emojiMap.put(
      "zombie_soldier_chestplate",
      "<:zombie_soldier_chestplate:816445031946780693>"
    );
    ...
    emojiMap.put(
      "night_vision_charm",
      "<:night_vision_charm:817495522133803010>"
    );
    emojiMap.put("cheetah_talisman", "<:cheetah_talisman:817495522201960470>");
    emojiMap.put(
      "red_claw_artifact",
      "<:red_claw_artifact:817495522410627072>"
    );
    emojiMap.put(
      "artifact_potion_affinity",
      "<:potion_artifact:817495522192392224>"
    );
    ...
    emojiMap.put(
      "enchanted_titanium",
      "<:enchanted_titanium:816457683703300168>"
    );
    emojiMap.put("farming_talisman", "<:farming_talisman:817495522407481414>");
    emojiMap.put(
      "skeleton_talisman",
      "<:skeleton_talisman:817495522390442025>"
    );
    emojiMap.put("lava_talisman", "<:lava_talisman:817495522457812992>");
    emojiMap.put(
      "fractured_mithril_pickaxe",
      "<a:fractured_mithril_pickaxe:816455519106629652>"
    );
    emojiMap.put(
      "auto_recombobulator",
      "<:auto_recombobulator:817495522369601627>"
    );
    ...
    emojiMap.put(
      ...
      "<:enchanted_slime_ball:816458357333557248>"
    );
    emojiMap.put(
      "sea_creature_talisman",
      "<:sea_creature_talisman:817495522520727582>"
    );
    emojiMap.put(
      "red_claw_talisman",
      "<:red_claw_talisman:817495522427797514>"
    );
    ...
    emojiMap.put(
      "radiant_power_orb",
      "<:radiant_power_orb:816457683699630150>"
    );
    ...
    emojiMap.put(
      "treasure_artifact",
      "<:treasure_artifact:817495522704752671>"
    );
    emojiMap.put(
      "hegemony_artifact",
      "<:hegemony_artifact:817495522389655602>"
    );
    emojiMap.put("mithril_ore", "<:mithril_ore:816457683615612989>");
    emojiMap.put(
      "scavenger_talisman",
      "<:scavenger_talisman:817495522381529138>"
    );
    emojiMap.put(
      "sea_creature_ring",
      "<:sea_creature_ring:817495522260156428>"
    );
    emojiMap.put("speed_ring", "<:speed_ring:817495522511945820>");
    emojiMap.put(
      "sea_creature_artifact",
      "<:seacreature_artifact:817495522067480588>"
    );
    emojiMap.put("glacite_jewel", "<:glacite_jewel:816457683707101234>");
    emojiMap.put("lucky_hoof", "<:lucky_hoof:817495522544713758>");
    emojiMap.put(
      "enchanted_mithril",
      "<:enchanted_mithril:816457683942244412>"
    );
    emojiMap.put(
      "seal_of_the_family",
      "<:seal_of_the_family:817495522377203732>"
    );
    emojiMap.put(
      "experience_artifact",
      "<:experience_artifact:817495522075213865>"
    );
    ...
    emojiMap.put(
      ...
      "<:fishing_talisman:817502293229436939>"
    );
    emojiMap.put(
      "magnetic_talisman",
      "<:magnetic_talisman:817502250187620362>"
    );
    ...
    emojiMap.put(
      ...
      "<:personal_compactor_4000:817502250313449502>"
    );
    ...
    emojiMap.put(
      "tarantula_talisman",
      "<:tarantula_talisman:817502250082762763>"
    );
    emojiMap.put(
      "titanium_artifact",
      "<:titanium_artifact:817502250334683136>"
    );
    ...
    emojiMap.put(
      ...
      "<:personal_compactor_5000:817502250309910539>"
    );
    ...
    emojiMap.put(
      ...
      "<:intimidation_artifact:817502250150658098>"
    );
    emojiMap.put("wolf_ring", "<:wolf_ring:817502250242015252>");
    emojiMap.put(
      "catacombs_expert_ring",
      "<:catacombs_expert_ring:817502293501935618>"
    );
    ...
    emojiMap.put(
      "intimidation_ring",
      "<:intimidation_ring:817502250117103637>"
    );
    ...
    emojiMap.put(
      ...
      "<:intimidation_talisman:817502250263511061>"
    );
    ...
    emojiMap.put(
      "treasure_talisman",
      "<:treasure_talisman:817502250347790416>"
    );
    ...
    emojiMap.put(
      ...
      "<:campfire_talisman_29:817868453805424700>"
    );
    ...
    emojiMap.put(
      "trick_or_treat_bag",
      "<:trick_or_treat_bag:817868453972934716>"
    );
    emojiMap.put(
      "super_magic_mushroom_soup",
      "<:super_magic_mushroom_soup:817868453599772695>"
    );
    ...
    emojiMap.put(
      "bat_person_artifact",
      "<:bat_person_artifact:817508883698417684>"
    );
    emojiMap.put("scarf_grimoir", "<:scarf_grimoir:817508883755958332>");
    emojiMap.put(
      "bat_person_talisman",
      "<:bat_person_talisman:817508883391447071>"
    );
    emojiMap.put(
      "sharp_shark_tooth_necklace",
      "<:sharp_shark_tooth_necklace:817869329479368745>"
    );
    ...
    emojiMap.put(
      ...
      "<:personal_deletor_4000:820521703506444289>"
    );
    ...
    emojiMap.put(
      "old_dragon_leggings",
      "<:old_dragon_leggings:820521703746699275>"
    );
    ...
    emojiMap.put(
      ...
      "<:personal_deletor_7000:820521703473021000>"
    );
    emojiMap.put(
      "jerry_talisman_golden",
      "<:jerry_talisman_golden:820521703418494996>"
    );
    emojiMap.put("fishing_rod", "<:fishing_rod:820521703028424705>");
    emojiMap.put(
      "razor_sharp_shark_tooth_necklace",
      "<:razor_sharp_shark_tooth_necklace:820521703536197642>"
    );
    emojiMap.put(
      "jerry_talisman_green",
      "<:jerry_talisman_green:820521703339065414>"
    );
    emojiMap.put("rancher_boots", "<:rancher_boots:820521703541047316>");
    emojiMap.put(
      "leather_chestplate",
      "<:leather_chestplate:820521703066173461>"
    );
    ...
    emojiMap.put(
      ...
      "<:pet_skin_monkey_golden:820521703091732491>"
    );
    ...
    emojiMap.put(
      ...
      "<:arachne_keeper_fragment:820521703116505088>"
    );
    emojiMap.put(
      "obsidian_chestplate",
      "<:obsidian_chestplate:820521704270725160>"
    );
    emojiMap.put(
      "pet_skin_sheep_neon_red",
      "<:pet_skin_sheep_neon_red:820521703620739084>"
    );
    emojiMap.put("healing_tissue", "<:healing_tissue:820521703309312050>");
    emojiMap.put(
      "diamond_chestplate",
      "<:diamond_chestplate:820521703246397490>"
    );
    emojiMap.put("soul_string", "<:soul_string:820536394988257321>");
    emojiMap.put("dark_orb", "<:dark_orb:820536334945878036>");
    emojiMap.put(
      "large_agronomy_sack",
      "<:large_agronomy_sack:820536335030157313>"
    );
    emojiMap.put(
      "old_dragon_helmet",
      "<:old_dragon_helmet:820536394509844480>"
    );
    ...
    emojiMap.put(
      ...
      "<:plasmaflux_power_orb:820536394481008670>"
    );
    emojiMap.put("kat_flower", "<:kat_flower:820536335134621746>");
    emojiMap.put(
      "giant_fragment_bigfoot",
      "<:giant_fragment_bigfoot:820536334812053525>"
    );
    ...
    emojiMap.put(
      "large_slayer_sack",
      "<:large_slayer_sack:820536335314976779>"
    );
    emojiMap.put("scarf_grimoire", "<:scarf_grimoire:820536394706583553>");
    emojiMap.put("venoms_touch", "<:venoms_touch:820536394673160242>");
    emojiMap.put(
      "large_foraging_sack",
      "<:large_foraging_sack:820536334984544298>"
    );
    emojiMap.put(
      "nether_wart_pouch",
      "<:nether_wart_pouch:820536335252586536>"
    );
    emojiMap.put(
      "old_dragon_chestplate",
      "<:old_dragon_chestplate:820536335227027456>"
    );
    ...
    emojiMap.put(
      ...
      "<:hoe_of_greater_tilling:820536335075901500>"
    );
    ...
    emojiMap.put(
      ...
      "<:large_husbandry_sack:820536335147335721>"
    );
    emojiMap.put(
      "zombie_commander_whip",
      "<:zombie_commander_whip:820536394790862858>"
    );
    ...
    emojiMap.put(
      "aspect_of_the_jerry",
      "<:aspect_of_the_jerry:820536394560307200>"
    );
    ...
    emojiMap.put(
      "wand_of_restoration",
      "<:wand_of_restoration:820536394854170654>"
    );
  ...
  public JsonArray getBankHistory() {
    try {
      return higherDepth(outerProfileJson, "banking.transactions")
        .getAsJsonArray();
    } catch (Exception e) {
      return null;
    }
  ...
  public int getSkillMaxLevel(String skillName) {
    if (skillName.equals("farming")) {
      return (
        higherDepth(getLevelingJson(), "leveling_caps." + skillName)
          .getAsInt() +
        getFarmingCapUpgrade()
      );
    }

    return higherDepth(getLevelingJson(), "leveling_caps." + skillName)
      .getAsInt();
  }
  ...
  public double getSkillXp(String skillName) {
    try {
      ...
      return higherDepth(profileJson, "experience_skill_" + skillName)
        .getAsDouble();
    } catch (Exception ignored) {}
    return -1;
  }
  ...
  public double getDungeonClassLevel(String className) {
    SkillsStruct dungeonClassLevel = skillInfoFromExp(
      getDungeonClassXp(className),
      "catacombs"
    );
  ...
  public double getDungeonClassXp(String className) {
    try {
      return higherDepth(
        profileJson,
        "dungeons.player_classes." + className + ".experience"
      )
        .getAsDouble();
    } catch (Exception e) {
      return 0;
  ...
  public int getDungeonSecrets() {
    ...
    try {
      return higherDepth(
        hypixelProfileJson,
        "player.achievements.skyblock_treasure_hunter"
      )
        .getAsInt();
    } catch (Exception e) {
      return 0;
  ...
  public String getSelectedDungeonClass() {
    try {
      return higherDepth(profileJson, "dungeons.selected_dungeon_class")
        .getAsString();
    } catch (Exception e) {
      return "none";
    }
  ...
  public String getFastestF7Time() {
    try {
      int f7TimeMilliseconds = higherDepth(
        profileJson,
  ...
  public EmbedBuilder defaultPlayerEmbed() {
    return defaultEmbed(
      fixUsername(getUsername()) +
      (higherDepth(outerProfileJson, "game_mode") != null ? " ♻️" : ""),
  ...
  public Map<Integer, InvItem> getInventoryMap() {
    try {
      String contents = higherDepth(profileJson, "inv_contents.data")
        .getAsString();
  ...
  public Map<Integer, InvItem> getStorageMap() {
    try {
      JsonElement backpackContents = higherDepth(
        profileJson,
        "backpack_contents"
      );
      ...
      for (String bp : backpackCount) {
        Collection<InvItem> curBpMap = getGenericInventoryMap(
          NBTReader.readBase64(
            higherDepth(backpackContents, bp + ".data").getAsString()
          )
        )
          .values();
  ...
  public Map<Integer, InvItem> getTalismanBagMap() {
    try {
      String contents = higherDepth(profileJson, "talisman_bag.data")
        .getAsString();
      NBTCompound parsedContents = NBTReader.readBase64(contents);
      return getGenericInventoryMap(parsedContents);
    } catch (Exception ignored) {}
  ...
  public Map<Integer, InvItem> getInventoryArmorMap() {
    try {
      String contents = higherDepth(profileJson, "inv_armor.data")
        .getAsString();
      NBTCompound parsedContents = NBTReader.readBase64(contents);
      return getGenericInventoryMap(parsedContents);
    } catch (Exception ignored) {}
  ...
  public Map<Integer, InvItem> getWardrobeMap() {
    try {
      String contents = higherDepth(profileJson, "wardrobe_contents.data")
        .getAsString();
      NBTCompound parsedContents = NBTReader.readBase64(contents);
      return getGenericInventoryMap(parsedContents);
    } catch (Exception ignored) {}
    ...
    for (JsonElement pet : petsArr) {
      try {
        ...
        invItemStruct.setName(
          capitalizeString(higherDepth(pet, "type").getAsString().toLowerCase())
        );
        invItemStruct.setId(
          higherDepth(pet, "type").getAsString() +
          rarityMap.get(higherDepth(pet, "tier").getAsString())
        );
        if (higherDepth(pet, "heldItem") != null) {
          invItemStruct.addExtraValue(
            higherDepth(pet, "heldItem").getAsString()
          );
        }
    ...
    for (JsonElement pet : petsArr) {
      try {
        ...
        invItemStruct.setName(
          "[Lvl " +
          petLevelFromXp(
            higherDepth(pet, "exp").getAsLong(),
            higherDepth(pet, "tier").getAsString().toLowerCase()
          ) +
          "] " +
          capitalizeString(
            higherDepth(pet, "type")
              .getAsString()
              .toUpperCase()
              .replace("_", " ")
          )
        ...
        if (
          higherDepth(pet, "skin") != null &&
          !higherDepth(pet, "skin").isJsonNull()
        ) {
          invItemStruct.addExtraValue(
            "PET_SKIN_" + higherDepth(pet, "skin").getAsString()
          );
        }
        invItemStruct.setRarity(higherDepth(pet, "tier").getAsString());
        if (
          higherDepth(pet, "heldItem") != null &&
          !higherDepth(pet, "heldItem").isJsonNull()
        ) {
          invItemStruct.addExtraValue(
            higherDepth(pet, "heldItem").getAsString()
          );
  ...
  public Map<Integer, InvItem> getEnderChestMap() {
    try {
      String contents = higherDepth(profileJson, "ender_chest_contents.data")
        .getAsString();
      NBTCompound parsedContents = NBTReader.readBase64(contents);
      return getGenericInventoryMap(parsedContents);
    } catch (Exception ignored) {}
