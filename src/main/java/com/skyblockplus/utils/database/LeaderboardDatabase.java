...
public class LeaderboardDatabase {
    ...
    private void insertIntoLeaderboard(Player player, Player.Gamemode gamemode) {
        try {
            ...
            getConnection().getCollection(gamemode.toCacheType()).updateOne(Filters.eq("uuid", player.getUuid()), Updates.combine(updates), new UpdateOptions().upsert(true));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public Map<String, Double> getLeaderboard(String lbType, Player.Gamemode mode) {
        try {
            ...
            FindIterable<Document> response = lbCollection.find().projection(Projections.include("username", lbType)).sort(Sorts.descending(lbType));
    ...
    public void updateLeaderboard() {
        try {
            FindIterable<Document> response = getConnection().getCollection("all_lb").find(Filters.lt("last_updated", Instant.now().minus(5, ChronoUnit.DAYS))).projection(Projections.include("uuid")).limit(180);
            int count = 0;
