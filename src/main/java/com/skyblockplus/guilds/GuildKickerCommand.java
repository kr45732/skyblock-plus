...
public class GuildKickerCommand extends Command {
  ...
  private EmbedBuilder getGuildKicker(
    String username,
    String reqs,
    CommandEvent event
  ) {
    ...
    if (reqsArr.length > 3) {
      return defaultEmbed("Error")
        .setDescription(
          "You can only enter a maximum of 3 sets of requirements"
        );
    }
    for (int i = 0; i < reqsArr.length; i++) {
      String[] indvReqs =
        reqsArr[i].replace("[", "").replace("]", "").split(" ");
      for (String indvReq : indvReqs) {
        String[] reqDashSplit = indvReq.split(":");
        if (reqDashSplit.length != 2) {
          return defaultEmbed("Error")
            .setDescription(indvReq + " is an invalid requirement format");
        }

        if (!"slayerskillscatacombsweight".contains(reqDashSplit[0])) {
          return defaultEmbed("Error")
            .setDescription(indvReq + " is an invalid requirement type");
        }
        ...
        try {
          ...
          return defaultEmbed("Error")
            .setDescription(indvReq + " is an invalid requirement value");
        }
      }
    ...
    if (guildJson != null && !higherDepth(guildJson, "guild").isJsonNull()) {
      ...
      if (higherDepth(guildLbJson, "data") != null) {
        ...
        for (JsonElement guildMember : guildMembers) {
          double slayer = higherDepth(guildMember, "total_slayer")
            .getAsDouble();
          double skills = higherDepth(guildMember, "average_skill_progress")
            .getAsDouble();
          double catacombs = higherDepth(guildMember, "catacomb").getAsDouble();
          double weight = higherDepth(guildMember, "raw_weight.total")
            .getAsDouble();
