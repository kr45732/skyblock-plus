...
public class GuildLeaderboardCommand extends Command {
  ...
  private EmbedBuilder getLeaderboard(String username, CommandEvent event) {
    ...
    if (
      !guildName.equals("Skyblock Forceful") &&
      !guildName.equals("Skyblock Gods")
    ) {
      ...
      lbSettings =
        higherDepth(
          JsonParser.parseReader(
            new FileReader(
              "src/main/java/com/skyblockplus/json/GuildSettings.json"
            )
          ),
          guildId + ".guild_leaderboard"
        );

      for (JsonElement i : higherDepth(lbSettings, "staff_ranks")
        .getAsJsonArray()) {
    ...
    for (JsonElement guildM : guildMembers) {
      ranksMap.put(
        higherDepth(guildM, "uuid").getAsString(),
    ...
    ArrayList<GuildLeaderboardStruct> guildCatacombs = new ArrayList<>(
      gMembers
    );
