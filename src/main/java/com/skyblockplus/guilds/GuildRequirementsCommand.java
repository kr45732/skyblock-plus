    ...
    new Thread(
      () -> {
        ...
        try {
          guildReqs =
            database
              .getApplyReqs(event.getGuild().getId(), args[1])
              .getAsJsonArray();
        } catch (Exception ignored) {}

        if (guildReqs == null) {
          List<AutomatedApplication> allGuilds = database.getAllApplySettings(
          ...
          try {
            ebStr =
              allGuilds.get(0).getName() + " or " + allGuilds.get(1).getName();
          } catch (Exception ignored) {}
        ...
        if (guildReqs == null || guildReqs.size() == 0) {
          ebMessage
            .editMessage(
              defaultEmbed("Error")
                .setDescription("No requirements set for " + args[1])
                .build()
            )
            .queue();
          return;
