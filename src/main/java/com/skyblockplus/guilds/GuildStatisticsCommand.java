...
public class GuildStatisticsCommand extends Command {
	...
	public static EmbedBuilder getStatistics(String username, Guild guild) {
		...
		List<String> slayerLb = guildMemberPlayersList.stream().sorted(Comparator.comparingDouble(m -> -getDoubleFromCache(m, "slayer"))).collect(Collectors.toList());
		List<String> skillsLb = guildMemberPlayersList.stream().sorted(Comparator.comparingDouble(m -> -getDoubleFromCache(m, "skills"))).collect(Collectors.toList());
		List<String> cataLb = guildMemberPlayersList.stream().sorted(Comparator.comparingDouble(m -> -getDoubleFromCache(m, "catacombs"))).collect(Collectors.toList());
		List<String> weightLb = guildMemberPlayersList.stream().sorted(Comparator.comparingDouble(m -> -getDoubleFromCache(m, "weight"))).collect(Collectors.toList());
		...
		eb.setDescription(
				"**Average slayer XP:** " + roundAndFormat(averageSlayer) +
				"\n**Average skills level:** " + roundAndFormat(averageSkills) +
				"\n**Average catacombs level:** " + roundAndFormat(averageCata) +
				"\n**Average weight:** " + roundAndFormat(averageWeight) +
						(
								lastUpdated != null
		...
		for (int i = 0; i < Math.min(5, slayerLb.size()); i++) {
			...
			slayerStr.append("`").append(i + 1).append(")` ").append(fixUsername(getStringFromCache(cur, "username"))).append(": ").append(roundAndFormat(getDoubleFromCache(cur, "slayer"))).append("\n");
		}
		StringBuilder skillsStr = new StringBuilder();
		for (int i = 0; i < Math.min(5, skillsLb.size()); i++) {
			String cur = skillsLb.get(i);
			skillsStr.append("`").append(i + 1).append(")` ").append(fixUsername(getStringFromCache(cur, "username"))).append(": ").append(roundAndFormat(getDoubleFromCache(cur, "skills"))).append("\n");
		}
		StringBuilder cataStr = new StringBuilder();
		for (int i = 0; i < Math.min(5, cataLb.size()); i++) {
			String cur = cataLb.get(i);
			cataStr.append("`").append(i + 1).append(")` ").append(fixUsername(getStringFromCache(cur, "username"))).append(": ").append(roundAndFormat(getDoubleFromCache(cur, "catacombs"))).append("\n");
		}
		StringBuilder weightStr = new StringBuilder();
		for (int i = 0; i < Math.min(5, weightLb.size()); i++) {
			String cur = weightLb.get(i);
			weightStr.append("`").append(i + 1).append(")` ").append(fixUsername(getStringFromCache(cur, "username"))).append(": ").append(roundAndFormat(getDoubleFromCache(cur, "weight"))).append("\n");
		}
