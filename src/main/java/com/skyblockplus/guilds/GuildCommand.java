...
public class GuildCommand extends Command {
  ...
  protected void execute(CommandEvent event) {
    new Thread(
      () -> {
        ...
        if (
          args.length == 3 &&
          ("experience".equals(args[1]) || "exp".equals(args[1]))
        ) {
          ...
          if (args[2].toLowerCase().startsWith("u:")) {
            ...
            ebMessage
              .editMessage(guildInfoFromGuildName(guildName).build())
              .queue();
            return;
          }
  ...
  private EmbedBuilder getGuildExp(String username, CommandEvent event) {
          ...
          uuidToUsernameResponse -> {
            try {
              String currentUsername = higherDepth(
                ...
                .getAsString();
              JsonElement expHistory = higherDepth(
                membersArr.get(finalI),
                "expHistory"
              );
    ...
    String rankStr =
      ...
      "\n**Guild Rank:** #" +
      (
        new ArrayList<>(reverseSortedMap.keySet())
          .indexOf(uuidUsername.playerUsername) +
        1
      ) +
      "\n**Exp:** " +
    ...
    for (Map.Entry<String, Integer> k : reverseSortedMap.entrySet()) {
      if (!k.getKey().startsWith("@null")) {
        paginateBuilder.addItems(
          ...
          (counter + 1) +
          ")` " +
          fixUsername(k.getKey()) +
          ": " +
          formatNumber(k.getValue()) +
          " EXP\n"
        );
      }
  ...
  private EmbedBuilder getGuildPlayer(String username) {
    ...
    try {
      ...
      eb.setThumbnail(
        "https://cravatar.eu/helmavatar/" + uuidUsername.playerUuid + "/64.png"
      );
      return eb;
    } catch (Exception e) {
  ...
  private String getGuildInfo(JsonElement guildJson) {
    ...
    String[] date = Date
      .from(Instant.ofEpochMilli(created.getAsLong()))
      .toString()
      .split(" ");
    guildInfo +=
      (
    ...
    for (int i = 0; i < guildMembers.size(); i++) {
      ...
      if (
        higherDepth(currentMember, "rank").getAsString().equals("Guild Master")
      ) {
        guildInfo +=
          (
            "• " +
    ...
    int numGuildMembers = higherDepth(guildJson, "guild.members")
      .getAsJsonArray()
      .size();
    guildInfo +=
      ("• " + guildName + " has " + numGuildMembers + " members") + "\n";
    JsonArray preferredGames;
    try {
      preferredGames =
        higherDepth(guildJson, "guild.preferredGames").getAsJsonArray();
    } catch (Exception e) {
    ...
    if (preferredGames.size() > 1) {
      ...
      prefString =
        prefString
          .substring(1, prefString.length() - 1)
          .toLowerCase()
          .replace("\"", "")
          .replace(",", ", ");
    ...
    guildInfo +=
      ("• " + guildName + " is guild level " + guildExpToLevel(guildExp)) +
      "\n";
  ...
  private EmbedBuilder getGuildMembers(String username, CommandEvent event) {
    ...
    JsonArray membersArr = higherDepth(guildJson, "guild.members")
      .getAsJsonArray();
  ...
  private int guildExpToLevel(int guildExp) {
    ...
    for (int i = 0;; i++) {
      int expNeeded = i >= guildExpTable.length
        ? guildExpTable[guildExpTable.length - 1]
        : guildExpTable[i];
      guildExp -= expNeeded;
      if (guildExp < 0) {
        return guildLevel;
