...
public class NetworthExecute {
          ...
          if (verbose) {
            try {
              if (calcItemsJsonStr.endsWith(",")) {
                calcItemsJsonStr =
                  calcItemsJsonStr.substring(0, calcItemsJsonStr.length() - 1);
              }
              calcItemsJsonStr += "]";
              ...
              nwEb.appendDescription(
                ...
                makeHastePost(
                  new GsonBuilder()
                    .setPrettyPrinting()
                    .create()
                    .toJson(JsonParser.parseString(calcItemsJsonStr))
                ) +
                ".json"
              );
            } catch (Exception e) {
  ...
  private EmbedBuilder getPlayerNetworth(String username, String profileName) {
    ...
    if (player.isValid()) {
      ...
      averageAuctionJson =
        getJson("https://moulberry.codes/auction_averages/3day.json");
      bazaarJson =
        higherDepth(
          getJson("https://api.hypixel.net/skyblock/bazaar"),
          "products"
        );
      sbzPrices =
        getJson(
          "https://raw.githubusercontent.com/skyblockz/pricecheckbot/master/data.json"
        )
          .getAsJsonArray();
      ...
      if (playerInventory == null) {
        return defaultEmbed(
          player.getUsername() + "'s inventory API is disabled"
        );
      }
      ...
      enderChestItems.sort(
        Comparator.comparingDouble(
          item -> -Double.parseDouble(item.split("@split@")[1])
        )
      );
      ...
      eb.addField(
        "Bank",
        (bankBalance == -1 ? "Private" : simplifyNumber(bankBalance)),
        true
      );
      eb.addField(
      ...
      eb.addField(
        ...
        storageStr.length() == 0 ? "Empty" : storageStr.toString(),
        false
      );
      eb.addField(
        "Inventory | " + simplifyNumber(invTotal),
        invStr.length() == 0 ? "Empty" : invStr.toString(),
        false
      );
      eb.addField(
        "Armor | " + simplifyNumber(invArmor),
        armorStr.length() == 0 ? "Empty" : armorStr.toString(),
        false
      );
      eb.addField(
        "Wardrobe | " + simplifyNumber(wardrobeTotal),
        wardrobeStr.length() == 0 ? "Empty" : wardrobeStr.toString(),
        false
      );
      eb.addField(
        "Pets | " + simplifyNumber(petsTotal),
        petsStr.length() == 0 ? "Empty" : petsStr.toString(),
        false
      );
      eb.addField(
        "Talisman | " + simplifyNumber(talismanTotal),
  ...
  private void calculateAllPetsPrice() {
    ...
    if (ahQuery != null) {
      for (JsonElement auction : ahQuery) {
        ...
        double auctionPrice = higherDepth(auction, "starting_bid")
          .getAsDouble();
        String auctionRarity = higherDepth(auction, "tier").getAsString();

        for (
          Iterator<InvItem> iterator = invPets.iterator();
          iterator.hasNext();
        ) {
          InvItem item = iterator.next();
          if (
            item.getName().equalsIgnoreCase(auctionName) &&
        ...
        for (
          Iterator<InvItem> iterator = petsPets.iterator();
          iterator.hasNext();
        ) {
          InvItem item = iterator.next();
          if (
        ...
        for (
          Iterator<InvItem> iterator = enderChestPets.iterator();
          iterator.hasNext();
        ) {
          InvItem item = iterator.next();
          if (
        ...
        for (
          Iterator<InvItem> iterator = storagePets.iterator();
          iterator.hasNext();
        ) {
          InvItem item = iterator.next();
          if (
    ...
    for (InvItem item : invPets) {
      try {
        double auctionPrice = higherDepth(
          lowestBinJson,
          item.getName().split("] ")[1].toLowerCase().trim() +
          rarityMap.get(item.getRarity())
        )
          .getAsDouble();
    ...
    return (
      ...
      (
        item.isRecombobulated()
          ? " <:recombobulator_3000:841689479621378139>"
          : ""
      ) +
      "@split@" +
      itemPrice
  ...
  private double calculateItemPrice(InvItem item, String location) {
    ...
    try {
      if (
        ...
        (
          ...
          2 >=
          higherDepth(
            higherDepth(bazaarJson, "RECOMBOBULATOR_3000.sell_summary")
              .getAsJsonArray()
              .get(0),
            "pricePerUnit"
          )
    ...
    try {
      hbpExtras =
        item.getHbpCount() *
        higherDepth(
          higherDepth(bazaarJson, "HOT_POTATO_BOOK.sell_summary")
            .getAsJsonArray()
            .get(0),
          "pricePerUnit"
        )
    ...
    try {
      fumingExtras =
        item.getFumingCount() *
        higherDepth(
          higherDepth(bazaarJson, "FUMING_POTATO_BOOK.sell_summary")
            .getAsJsonArray()
            .get(0),
          "pricePerUnit"
        )
    ...
    try {
      ...
      for (String enchant : enchants) {
        try {
          if (
            item.getDungeonFloor() != 0 &&
            enchant.equalsIgnoreCase("scavenger;5")
          ) {
            continue;
          }
    ...
    try {
      reforgeExtras =
        calculateReforgePrice(item.getModifier(), item.getRarity());
    } catch (Exception ignored) {}
    ...
    if (verbose) {
      ...
      calcItemsJsonStr +=
        recombobulatedExtra > 0
          ? ",\"recomb\":\"" + simplifyNumber(recombobulatedExtra) + "\""
          : "";
      calcItemsJsonStr +=
        (hbpExtras > 0 ? ",\"hbp\":\"" + simplifyNumber(hbpExtras) + "\"" : "");
      calcItemsJsonStr +=
        (
          enchantsExtras > 0
            ? ",\"enchants\":{\"total\":\"" +
            simplifyNumber(enchantsExtras) +
            "\",\"enchants\":" +
            enchStr +
            "}"
            : ""
      ...
      calcItemsJsonStr +=
        ",\"nbt_tag\":\"" +
        parseMcCodes(item.getNbtTag().replace("\"", "\\\"")) +
        "\"";
      calcItemsJsonStr += "},";
    }
  ...
  private double calculateReforgePrice(String reforgeName, String itemRarity) {
    ...
    for (String reforgeStone : reforgeStones) {
      JsonElement reforgeStoneInfo = higherDepth(
        reforgesStonesJson,
        reforgeStone
      );
      if (
        higherDepth(reforgeStoneInfo, "reforgeName")
          .getAsString()
          .equalsIgnoreCase(reforgeName)
      ) {
        String reforgeStoneName = higherDepth(reforgeStoneInfo, "internalName")
          .getAsString();
        double reforgeStoneCost = getLowestPrice(reforgeStoneName, " ");
        double reforgeApplyCost = higherDepth(
          reforgeStoneInfo,
          "reforgeCosts." + itemRarity.toUpperCase()
        )
          .getAsDouble();
  ...
  private double getLowestPriceEnchant(String enchantId) {
    ...
    for (int i = enchantLevel; i >= 1; i--) {
      try {
        lowestBin =
          higherDepth(lowestBinJson, enchantName + ";" + i).getAsDouble();
      } catch (Exception ignored) {}

      try {
        JsonElement avgInfo = higherDepth(
          averageAuctionJson,
          enchantName + ";" + i
        );
        averageAuction =
      ...
      if (lowestBin == -1 && averageAuction != -1) {
        ...
        return (
          Math.pow(2, enchantLevel - i) * Math.min(lowestBin, averageAuction)
        );
      }
    }
    ...
    if (higherDepth(sbzPrices, enchantName + "_1") != null) {
      return (
        Math.pow(2, enchantLevel - 1) *
        higherDepth(sbzPrices, enchantName + "_1").getAsDouble()
      );
    }
    ...
    if (higherDepth(sbzPrices, enchantName + "_i") != null) {
      return (
        Math.pow(2, enchantLevel - 1) *
        higherDepth(sbzPrices, enchantName + "_i").getAsDouble()
      );
    }
  ...
  private double getLowestPrice(String itemId, String tempName) {
    ...
    try {
      return higherDepth(
        higherDepth(bazaarJson, itemId + "sell_summary")
          .getAsJsonArray()
          .get(0),
        "pricePerUnit"
      )
    ...
    try {
      ...
      if (itemId.contains("generator")) {
        ...
        for (JsonElement itemPrice : sbzPrices) {
          ...
          if (
            itemNamePrice.startsWith("tier_" + hoeLevel) &&
      ...
      for (JsonElement itemPrice : sbzPrices) {
        if (
          higherDepth(itemPrice, "name").getAsString().equalsIgnoreCase(itemId)
        ) {
          return higherDepth(itemPrice, "low").getAsDouble();
        }
      }
