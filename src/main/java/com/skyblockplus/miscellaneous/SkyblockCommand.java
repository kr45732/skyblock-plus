...
public class SkyblockCommand extends Command {
  ...
  private EmbedBuilder getSkyblockStats(
    String username,
    String profileName,
    CommandEvent event
  ) {
    ...
    if (player.isValid()) {
      ...
      generalPageString +=
        ...
        dateFormatter.format(
          Instant.ofEpochMilli(
            higherDepth(profileJson, "first_join").getAsLong()
          )
        ) +
        "\n";
      generalPageString +=
        "**Purse:** " + simplifyNumber(player.getPurseCoins()) + "\n";
      generalPageString +=
        "**Bank Account:** " +
        (
          player.getBankBalance() == -1.0
            ? "Banking API disabled"
            : simplifyNumber(player.getBankBalance())
        ) +
        "\n";
      generalPageString +=
        "**Skill Average:** " + roundAndFormat(player.getSkillAverage()) + "\n";
      generalPageString +=
        "**Fairy Souls:** " + player.getFairySouls() + " / 220" + "\n";
      ...
      armorPageString +=
        "**Equipped Chestplate:** " + invArmor.getChestplate() + "\n";
      armorPageString +=
        "**Equipped Leggings:** " + invArmor.getLeggings() + "\n";
      armorPageString += "**Equipped Boots:** " + invArmor.getBoots() + "\n";
      armorPageString +=
        "**Wardrobe:** Use `" +
        BOT_PREFIX +
        "wardrobe` to view wardrobe armor" +
        "\n";
      ...
      skillsPageString +=
        "• **Contests Attended:** " +
        getJsonKeys(higherDepth(jacobJson, "contests")).size() +
        "\n";
      skillsPageString +=
        "• **Unique Golds:** " +
        (
          higherDepth(jacobJson, "unique_golds2") != null
      ...
      skillsPageString +=
        "• **Current Golds:** " +
        (
          higherDepth(jacobJson, "medals_inv.gold") == null
            ? 0
      ...
      skillsPageString +=
        "• **Current Silvers:** " +
        (
          higherDepth(jacobJson, "medals_inv.silver") == null
            ? 0
      ...
      skillsPageString +=
        "• **Current Bronzes:** " +
        (
          higherDepth(jacobJson, "medals_inv.bronze") == null
            ? 0
      ...
      skillsPageString +=
        "• **Items Fished:** " +
        formatNumber(higherDepth(statsJson, "items_fished").getAsInt()) +
        "\n";
      skillsPageString +=
        "• **Treasures Fished:** " +
        formatNumber(
          higherDepth(statsJson, "items_fished_treasure").getAsInt()
        ) +
        "\n";
      skillsPageString +=
        "• **Large Treasures Fished:** " +
        formatNumber(
          higherDepth(statsJson, "items_fished_large_treasure").getAsInt()
        ) +
        "\n";
      skillsPageString +=
      ...
      dungeonsPageString +=
        "• **Healer:** " +
        roundAndFormat(player.getDungeonClassLevel("healer")) +
        "\n";
      dungeonsPageString +=
        "• **Mage:** " +
        roundAndFormat(player.getDungeonClassLevel("mage")) +
        "\n";
      dungeonsPageString +=
        "• **Berserk:** " +
        roundAndFormat(player.getDungeonClassLevel("berserk")) +
        "\n";
      dungeonsPageString +=
        "• **Archer:** " +
        roundAndFormat(player.getDungeonClassLevel("archer")) +
        "\n";
      dungeonsPageString +=
        "• **Tank:** " +
        roundAndFormat(player.getDungeonClassLevel("tank")) +
        "\n";
      dungeonsPageString += "\n";

      dungeonsPageString += "__**Catacombs**__" + "\n";
      dungeonsPageString +=
        "• **Catacombs:** " + roundAndFormat(player.getCatacombsLevel()) + "\n";
