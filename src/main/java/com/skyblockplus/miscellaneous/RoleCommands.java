  ...
  protected void execute(CommandEvent event) {
    new Thread(
      () -> {
        ...
        JsonElement linkedInfo = database.getLinkedUserByDiscordId(
          user.getId()
        );
        if (
          getPlayerDiscordInfo(
            higherDepth(linkedInfo, "minecraftUuid").getAsString()
          ) ==
          null
        ) {
          eb = defaultEmbed("Discord tag error");
          eb.setDescription(
            "Unable to get Discord tag linked with Hypixel account"
          );
          ebMessage.editMessage(eb.build()).queue();
          return;
        }
        DiscordInfoStruct playerInfo = getPlayerDiscordInfo(
          higherDepth(linkedInfo, "minecraftUuid").getAsString()
        );
        ...
        if (!player.isValid()) {
          ...
          eb.setDescription(
            "**Please check linked username or given profile**"
          );
        ...
        if (args[1].equals("claim")) {
          try {
            ...
            if (rolesJson != null) {
              if (
                (higherDepth(rolesJson, "enable") == null) ||
                !higherDepth(rolesJson, "enable").getAsBoolean()
              ) {
                eb =
                  defaultEmbed("Automatic roles not enabled for this server");
                ebMessage.editMessage(eb.build()).queue();
                return;
              }
              ...
              for (String currentRoleName : rolesID) {
                JsonElement currentRole = higherDepth(
                  rolesJson,
                  currentRoleName
                );
                if (
                  (higherDepth(currentRole, "enable") == null) ||
                  ...
                  case "guild_member":
                    {
                      ...
                      if (
                        guildJson != null &&
                        !higherDepth(guildJson, "guild").isJsonNull()
                      ) {
                        JsonArray levelsArray = higherDepth(
                          currentRole,
                          "levels"
                        )
                          .getAsJsonArray();
                        String playerGuildId = higherDepth(
                          guildJson,
                          "guild._id"
                        )
                          .getAsString();
                        ...
                        for (JsonElement currentLevel : levelsArray) {
                          String currentLevelValue = higherDepth(
                            currentLevel,
                            "value"
                          )
                            .getAsString();
                          ...
                          if (playerGuildId.equals(currentLevelValue)) {
                            if (!member.getRoles().contains(currentLevelRole)) {
                              if (botRole.canInteract(currentLevelRole)) {
                                guild
                                  .addRoleToMember(member, currentLevelRole)
                                  .queue();
                                addedRoles.append(
                                  roleChangeString(currentLevelRole.getName())
                                );
                              } else {
                                errorRoles.append(
                                  roleChangeString(currentLevelRole.getName())
                  ...
                  case "guild_ranks":
                    {
                      ...
                      if (
                        ...
                        JsonArray curLevels = higherDepth(currentRole, "levels")
                          .getAsJsonArray();
                        List<JsonElement> guildRoles = new ArrayList<>();
                        for (JsonElement curLevel : curLevels) {
                          guildRoles.add(
                            database.getGuildRoleSettings(
                              guild.getId(),
                        ...
                        for (JsonElement guildRoleSettings : guildRoles) {
                          if (
                            higherDepth(guildRoleSettings, "guildId")
                              .getAsString()
                              .equals(
                                higherDepth(guildJson, "guild._id")
                                  .getAsString()
                              )
                          ) {
                            JsonArray guildRanks = higherDepth(
                              guildRoleSettings,
                              "guildRanks"
                            )
                              .getAsJsonArray();

                            JsonArray guildMembers = higherDepth(
                              guildJson,
                              "guild.members"
                            )
                              .getAsJsonArray();
                            ...
                            for (JsonElement guildMember : guildMembers) {
                              if (
                                ...
                                String guildMemberRank = higherDepth(
                                  guildMember,
                                  "rank"
                                )
                                  .getAsString()
                                ...
                                for (JsonElement guildRank : guildRanks) {
                                  Role currentLevelRole = guild.getRoleById(
                                    higherDepth(guildRank, "discordRoleId")
                                      .getAsString()
                                  );
                                  if (
                                    ...
                                    if (
                                      !member
                                        .getRoles()
                                        .contains(currentLevelRole)
                                    ) {
                                      if (
                                        botRole.canInteract(currentLevelRole)
                                      ) {
                                        guild
                                          .addRoleToMember(
                                            member,
                  ...
                  case "slot_collector":
                    {
                        ...
                        case "bank_coins":
                          {
                            ...
                            if (
                              roleAmount == -1 &&
                              !disabledAPI.toString().contains("Banking")
                            ) {
                              disabledAPI.append(
                                roleChangeString("Banking API disabled")
                              );
                              continue;
                            }
                            break;
                        ...
                        case "skill_average":
                          {
                            ...
                            if (
                              roleAmount == -1 &&
                              !disabledAPI.toString().contains("Skills")
                            ) {
                              disabledAPI.append(
                                roleChangeString("Skills API disabled")
                              );
                              continue;
                            }
                            break;
                        ...
                        case "enchanting":
                          {
                            if (player.getSkill(currentRoleName) != null) {
                              roleAmount =
                                player.getSkill(currentRoleName).skillLevel;
                            }
                            if (
                              roleAmount == -1 &&
                              !disabledAPI.toString().contains("Skills")
                        ...
                        case "catacombs":
                          {
                            if (player.getCatacombsSkill() != null) {
                              roleAmount =
                                player.getCatacombsSkill().skillLevel;
                            }
                            if (
                              roleAmount == -1 &&
                              !disabledAPI.toString().contains("Skills")
                      ...
                      JsonArray levelsArray = higherDepth(currentRole, "levels")
                        .getAsJsonArray();

                      if (
                        higherDepth(currentRole, "stackable").getAsBoolean()
                      ) {
                        for (JsonElement currentLevel : levelsArray) {
                          int currentLevelValue = higherDepth(
                            currentLevel,
                            "value"
                          )
                            .getAsInt();
                          Role currentLevelRole = event
                            .getGuild()
                            .getRoleById(
                              higherDepth(currentLevel, "roleId").getAsString()
                            );
                        ...
                        for (int i = levelsArray.size() - 1; i >= 0; i--) {
                          ...
                          if (roleAmount < currentLevelValue) {
                            ...
                            for (int j = i - 1; j >= 0; j--) {
                              JsonElement currentLevelRemoveStackable = levelsArray.get(
                                j
                              );
                              Role currentLevelRoleRemoveStackable = event
                                .getGuild()
                                .getRoleById(
                                  higherDepth(
                                    currentLevelRemoveStackable,
                                    "roleId"
                                  )
                                    .getAsString()
                                );
                              ...
                              if (
                                member
                                  .getRoles()
                                  .contains(currentLevelRoleRemoveStackable)
                              ) {
                                if (botRole.canInteract(currentLevelRole)) {
                                  guild
                                    .removeRoleFromMember(
                                      member,
                                      currentLevelRoleRemoveStackable
                                    )
                                    .queue();
                                  removedRoles.append(
                                    roleChangeString(
                  ...
                  case "doom_slayer":
                    {
                      Role curRole = guild.getRoleById(
                        higherDepth(
                          higherDepth(currentRole, "levels")
                            .getAsJsonArray()
                            .get(0),
                          "roleId"
                        )
                      ...
                      if (
                        ...
                        if (!member.getRoles().contains(curRole)) {
                          if (botRole.canInteract(curRole)) {
                            ...
                            addedRoles.append(
                              roleChangeString(curRole.getName())
                            );
                          } else {
                            errorRoles.append(
                              roleChangeString(curRole.getName())
                  ...
                  case "pet_enthusiast":
                    {
                      ...
                      for (JsonElement currentPet : playerPets) {
                        String currentPetRarity = higherDepth(
                          currentPet,
                          "tier"
                        )
                          .getAsString()
                          .toLowerCase();
                        if (
                          currentPetRarity.equals("epic") ||
                          currentPetRarity.equals("legendary")
                        ) {
                          if (
                            !excludedPets.contains(
                              higherDepth(currentPet, "type")
                                .getAsString()
                                .toLowerCase()
                            )
                          ) {
                            long currentPetExp = higherDepth(currentPet, "exp")
                              .getAsLong();
                            if (
                              player.petLevelFromXp(
                                currentPetExp,
                                currentPetRarity
                              ) ==
                              100
                              ...
                              if (
                                !member.getRoles().contains(petEnthusiastRole)
                              ) {
                                if (botRole.canInteract(petEnthusiastRole)) {
                                  guild
                                    .addRoleToMember(member, petEnthusiastRole)
                                    .queue();
                                  addedRoles.append(
                                    roleChangeString(
                                      petEnthusiastRole.getName()
                                    )
                                  );
                                } else {
                                  errorRoles.append(
                      ...
                      if (
                        ...
                        if (botRole.canInteract(petEnthusiastRole)) {
                          ...
                          guild
                            .removeRoleFromMember(member, petEnthusiastRole)
                            .queue();
                        } else {
                          errorRoles.append(
                            roleChangeString(petEnthusiastRole.getName())
