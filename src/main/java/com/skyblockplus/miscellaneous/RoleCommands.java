		...
		new Thread(
			() -> {
				...
				if (database.getLinkedUserByDiscordId(user.getId()).isJsonNull()) {
					ebMessage
						.editMessage(
							defaultEmbed("You must be linked to run this command. Use `" + BOT_PREFIX + "link [IGN]` to link").build()
						)
						.queue();
					return;
				}
																	...
																	if (
																		...
																		if (member.getRoles().contains(currentLevelRole)) {
																			if (botRole.canInteract(currentLevelRole)) {
																				...
																				guild
																					.removeRoleFromMember(member, currentLevelRole)
																					.queue();
																			} else {
																				errorRoles.append(
																					roleChangeString(currentLevelRole.getName())
													...
													if (roleAmount < currentLevelValue) {
														...
														for (int j = i - 1; j >= 0; j--) {
															...
															if (member.getRoles().contains(currentLevelRoleRemoveStackable)) {
																if (botRole.canInteract(currentLevelRole)) {
																	guild
																		.removeRoleFromMember(member, currentLevelRoleRemoveStackable)
																		.queue();
																	removedRoles.append(
																		roleChangeString(currentLevelRoleRemoveStackable.getName())
																	);
																} else {
																	errorRoles.append(
																		roleChangeString(currentLevelRoleRemoveStackable.getName())
									...
									case "doom_slayer":
										{
											Role curRole = guild.getRoleById(
												higherDepth(higherDepth(currentRole, "levels").getAsJsonArray().get(0), "roleId")
													.getAsString()
											);
									...
									case "pet_enthusiast":
										{
											...
											for (JsonElement currentPet : playerPets) {
												...
												if (currentPetRarity.equals("epic") || currentPetRarity.equals("legendary")) {
													if (
														!excludedPets.contains(higherDepth(currentPet, "type").getAsString().toLowerCase())
													) {
