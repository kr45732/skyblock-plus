...
public class BestiaryCommand extends Command {
	...
	public static final Map<String, List<String>> locations =
			Maps.of(
							"Private Island",
							List.of(
									"family_cave_spider",
							...
							"family_worms",
							"family_yog"
					),"The Park", List.of("family_howling_spirit", "family_pack_spirit", "family_soul_of_the_alpha"
					),"Spooky", List.of(
							"family_batty_witch",
	...
	public static final List<String> bosses = List.of("family_arachne",
			"family_brood_mother_spider",
			"family_dragon",
			"family_corrupted_protector",
			"family_ashfang",
		...
		if (player.isValid()) {
			...
			for (Map.Entry<String, List<String>> location : locations.entrySet()) {
				...
				for (String mob : location.getValue()) {
					...
					SkillsStruct level = levelingInfoFromExp(kills, "bestiary." + type,higherDepth(getLevelingJson(), "bestiary.caps." + type).getAsInt());
					eb.addField(mobName, "Level: " + roundAndFormat(level.getProgressLevel()) + "\nKills: " + formatNumber(kills) + ((level.expForNext() - level.expCurrent() > 0) ?  "\nNext: " + formatNumber(level.expForNext() - level.expCurrent()) : ""), true);
				}
		...
		new CommandExecute(this, event) {
			...
			protected void execute() {
				...
				if (args.length == 3 || args.length == 2 || args.length == 1) {
					...
					paginate(getBestiary(player, args.length == 3 ? args[2] : null, getPaginatorEvent()));
					return;
				}
