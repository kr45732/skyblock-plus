			...
			if (higherDepth(hypixelJson, "player") != null) {
				...
				try {
					...
					for (String parkourLocation : getJsonKeys(higherDepth(hypixelJson, "parkourCompletions"))) {
						int fastestTime = -1;
						for (JsonElement parkourTime : higherDepth(hypixelJson, "parkourCompletions." + parkourLocation).getAsJsonArray()) {
			...
			if (higherDepth(hypixelJson, "player") != null) {
				...
				try {
					for (String socialMedia : getJsonKeys(higherDepth(hypixelJson, "socialMedia.links"))) {
						String currentSocialMediaLink = higherDepth(higherDepth(hypixelJson, "socialMedia.links"), socialMedia)
							.getAsString();
						eb.addField(
				...
				for (JsonElement name : higherDepth(
					getJson("https://api.ashcon.app/mojang/v2/user/" + usernameUuid.playerUuid),
					"username_history"
				)
					.getAsJsonArray()) {
