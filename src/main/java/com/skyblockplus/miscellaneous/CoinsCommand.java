...
public class CoinsCommand extends Command {
    ...
    public CoinsCommand(EventWaiter waiter) {
        this.name = "coins";
        ...
        if (player.isValid()) {
            ...
            if (bankHistoryArray != null) {
                ...
                for (int i = bankHistoryArray.size() - 1; i >= 0; i--) {
                    ...
                    String valueString = simplifyNumber(higherDepth(currentTransaction, "amount").getAsLong()) + " " + (higherDepth(currentTransaction, "action").getAsString().equals("DEPOSIT") ? "deposited" : "withdrawn") + " by " + higherDepth(currentTransaction, "initiator_name").getAsString().replaceAll("§f|§a|§9|§5|§6|§d|§4|§c|§7|§b", "");
