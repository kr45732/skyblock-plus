...
public class BankCommand extends Command {
  ...
  protected void execute(CommandEvent event) {
    new Thread(
      () -> {
        ...
        if (
          (args.length == 3 || args.length == 4) && args[1].equals("history")
        ) {
          if (args.length == 4) {
          ...
          ebMessage
            .editMessage(getPlayerBalance(args[1], null).build())
            .queue();
          return;
        } else if (args.length == 3) {
          ebMessage
            .editMessage(getPlayerBalance(args[1], args[2]).build())
            .queue();
          return;
        }
    ...
    if (player.isValid()) {
      ...
      eb.addField(
        ...
        playerBankBalance == -1
          ...
          : simplifyNumber(playerBankBalance) + " coins",
        false
      );
      eb.addField(
        "Purse coins",
  ...
  private EmbedBuilder getPlayerBankHistory(
    String username,
    String profileName,
    CommandEvent event
  ) {
    ...
    if (player.isValid()) {
      ...
      if (bankHistoryArray != null) {
        ...
        paginateBuilder.addItems(
          ...
          dateTimeFormatter.format(
            Instant.ofEpochMilli(
              higherDepth(
                bankHistoryArray.get(bankHistoryArray.size() - 1),
                "timestamp"
              )
                .getAsLong()
            )
        ...
        for (int i = bankHistoryArray.size() - 1; i >= 0; i--) {
          ...
          String valueString =
            simplifyNumber(
              higherDepth(currentTransaction, "amount").getAsLong()
            ) +
            " " +
            (
              higherDepth(currentTransaction, "action")
                  .getAsString()
                  .equals("DEPOSIT")
                ? "deposited"
                : "withdrawn"
            ...
            " by " +
            parseMcCodes(
              higherDepth(currentTransaction, "initiator_name").getAsString()
            );

          String time = dateTimeFormatter.format(
            Instant.ofEpochMilli(
              higherDepth(currentTransaction, "timestamp").getAsLong()
            )
          );
