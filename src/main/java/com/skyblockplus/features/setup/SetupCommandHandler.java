			...
			case "guild":
				buttonEvent
						...
						.editOriginalEmbeds(
								defaultEmbed("Setup")
										.setDescription(
												"Reply with the name of the Hypixel guild you are setting this up for.")
										.setFooter("Reply with 'cancel' to stop the process")
										.setFooter("Reply with 'cancel' to stop the process")
										.build())
						.queue();
				...
				if (featureType.startsWith("guild_apply")) {
					...
					buttonEvent
							...
							.editOriginalEmbeds(
									defaultEmbed("Setup")
											.setDescription(
													"Reply with the message that users will see and click to in order to apply.")
											.setFooter("Reply with 'cancel' to stop the process")
											.build())
							.queue();
						...
						if (event.getMessage().getContentRaw().equalsIgnoreCase("enable")) {
							...
							if (eb.build().getTitle().equals("Settings")) {
								...
								if (msg.equals("Reloaded")) {
									sendEmbed(
											defaultEmbed("Success").setDescription("Enabled automatic verification."));
								} else {
			...
			case "guild":
				switch (state) {
					case 0:
						...
						if (eb.build().getTitle().equals("Settings")) {
							this.name = eb.build().getDescription().split("`")[1].split("`")[0].toLowerCase()
									.replace(" ", "_");
							eb = defaultEmbed("Setup")
									.setDescription(
							...
							buttonEvent
									...
									.setActionRow(
											...
											Button.primary("setup_command_guild_counter_" + name,
					...
					case 10:
						if (!event.getMessage().getContentRaw().equalsIgnoreCase("none")) {
							...
							if (reqs.length == 0 || reqs.length > 3) {
								eb = defaultEmbed(
										"You must add at least one requirement and at most three requirements");
							} else {
