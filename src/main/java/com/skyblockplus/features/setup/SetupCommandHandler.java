...
public class SetupCommandHandler {
	...
	public SetupCommandHandler(ButtonClickEvent buttonEvent, String featureType) {
		settings = new SettingsExecute(buttonEvent.getGuild(), buttonEvent.getMessage(), buttonEvent.getChannel(),
				buttonEvent.getUser());
		...
		case "verify":
			buttonEvent.getHook()
					.editOriginalEmbeds(defaultEmbed("Setup")
							.setDescription("Reply with the message that users will see when verifying.")
							.setFooter("Reply with 'cancel' to stop the process").build())
					.queue();
			break;
		case "guild":
			buttonEvent.getHook()
			...
			break;
		case "roles":
			buttonEvent.getHook().editOriginalEmbeds(defaultEmbed("Setup").setDescription("**__Overview__**\n"
		...
		default:
			if (featureType.startsWith("guild_apply")) {
				...
				buttonEvent.getHook()
						.editOriginalEmbeds(defaultEmbed("Setup").setDescription("Reply with the guild member role.")
								.setFooter("Reply with 'cancel' to stop the process").build())
						.queue();
				...
				if (eb.build().getTitle().equals("Settings")) {
					buttonEvent.getHook()
							.editOriginalEmbeds(
									defaultEmbed("Success").setDescription("Enabled guild member counter.").build())
							.queue();
				} else {
	...
	private boolean checkReply(GuildMessageReceivedEvent event) {
		return (event.getChannel().getId().equals(buttonEvent.getChannel().getId())
	...
	private void handleReply(GuildMessageReceivedEvent event) {
		...
		case "guild":
			...
			case 0:
				...
				if (eb.build().getTitle().equals("Settings")) {
					...
					eb = defaultEmbed("Setup").setDescription(
							"Choose one of the buttons below to setup the corresponding automatic guild feature");
					buttonEvent.getChannel().sendMessageEmbeds(eb.build())
							.setActionRow(Button.primary("setup_command_guild_apply_" + name, "Automated Apply"),
		...
		case "guild_apply":
			...
			case 8:
				...
				break;
			case 9:
				eb = settings.setApplyIronman(getSettings(),
				...
				if (event.getMessage().getContentRaw().equalsIgnoreCase("enable")) {
					...
					if (eb.build().getTitle().equals("Settings")) {
						...
						if (msg.contains("• Reloaded `" + name + "`")) {
							sendEmbed(defaultEmbed("Success")
									.setDescription("Enabled this automatic application systen."));
						} else {
							if (!msg.contains("• `" + name + "` is disabled")) {
								msg = "`" + name + "` is disabled";
							} else {
								msg = "Error Reloading for `" + name
										+ msg.split("• Error Reloading for `" + name)[1].split("\n")[0];
							}
							sendEmbed(defaultEmbed("Error").setDescription(msg));
						}
					} else {
		...
		case "guild_ranks":
			...
			if (guildRanks.length == 0) {
				...
				for (String guildRank : guildRanks) {
					...
					eb = settings.addGuildRank(getSettings(), guildRanksSplit.length >= 1 ? guildRanksSplit[0] : null,
	...
	private void waitForReply() {
		waiter.waitForEvent(GuildMessageReceivedEvent.class, this::checkReply, this::handleReply, 1, TimeUnit.MINUTES,
				() -> {
					cancel();
