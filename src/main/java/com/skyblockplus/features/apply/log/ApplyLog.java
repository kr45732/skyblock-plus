...
public class ApplyLog {
    ...
    public static LogMessage toLog(Message message) {
        LogMessage.Builder builder = LogMessage
                ...
                .user(
                        LogUser
                                ...
                                .color(toHex(message.getMember().getColorRaw())).build()
                );

        String text = message.getContentRaw();
        for (Member mentionedMember : message.getMentionedMembers()) {
            text = text.replace("<@" + mentionedMember.getId() + ">", "<discord-mention>" + mentionedMember.getEffectiveName() + "</discord-mention>").replace("<@!" + mentionedMember.getId() + ">", "<discord-mention>" + mentionedMember.getEffectiveName() + "</discord-mention>");
        }
        for (TextChannel mentionedChannel : message.getMentionedChannels()) {
            text = text.replace(mentionedChannel.getAsMention(), "<discord-mention type=\"channel\">" + mentionedChannel.getName() + "</discord-mention>");
        }
        for (Role mentionedRole : message.getMentionedRoles()) {
            text = text.replace(mentionedRole.getAsMention(), "<discord-mention type=\"role\" color=\"" + toHex(mentionedRole.getColorRaw()) + "\">" + mentionedRole.getName() + "</discord-mention>");
        }
        ...
        for (MessageEmbed embed : message.getEmbeds()) {
            ...
            for (PartialEmote emote : getEmotes(description)) {
                description = description.replace(emote.getAsMention(), "<img src=\"" + emote.getImageUrl() + "\" width=\"16\" height=\"16\"/>");
            }
            LogEmbed.Builder embedBuilder = LogEmbed.builder()
    ...
    public static <T, C extends Collection<T>> C processMentions(String content, Message.MentionType type, C collection, Function<Matcher, T> map) {
    ...
    public static String parseMarkdown(String str, String regex, String startTag, String endTag){
        return Pattern.compile("(?<!\\\\)" + regex  + "(.*)" + "(?<!\\\\)" + regex).matcher(str).replaceAll(match -> "<" + startTag + ">" + match.group(1) + "</" + endTag + ">");
    }
