...
public class EventGuild {
    ...
    public void onEventNotif(Map<String, MessageEmbed> embeds) {
        try {
            if (enable) {
                List<MessageEmbed> filteredEmbeds = embeds.entrySet().stream().filter(e -> wantedEvents.contains(e.getKey())).map(Map.Entry::getValue).collect(Collectors.toList());
    ...
    public void reloadSettingsJson(JsonElement eventSettings) {
        try {
            ...
            if (enable) {
                ...
                wantedEvents = streamJsonArray(higherDepth(eventSettings, "events").getAsJsonArray()).map(JsonElement::getAsString).collect(Collectors.toList());
            }
        } catch (Exception e) {
            AutomaticGuild.getLogger().error(guildId, e);
        }
