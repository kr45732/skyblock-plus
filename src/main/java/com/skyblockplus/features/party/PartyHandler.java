...
public class PartyHandler {
    ...
    public PartyHandler(String username, PaginatorEvent paginatorEvent){
        ...
        this.message = paginatorEvent.getChannel().sendMessageEmbeds(defaultEmbed("Party Finder Creator").setDescription("Choose an option from the menu below to get started!").build()).setActionRow(menu).complete();
        scheduleWaiter();
    }

    private boolean condition(SelectionMenuEvent event){
        return event.getUser().getId().equals(paginatorEvent.getUser().getId()) && event.getChannel().getId().equals(paginatorEvent.getChannel().getId()) && event.getComponentId().equals(menuId);
    }
        ...
        switch (menuId){
            case "party_finder_create_main":
                switch (event.getSelectedOptions().get(0).getValue()){
                    case "floor":
                        SelectionMenu floorMenu = SelectionMenu.create("party_finder_create_floor")
                        ...
                        event.getHook().editOriginalEmbeds(getCreationEmbed().setDescription("Choose a floor from the menu below").build()).setActionRow(floorMenu).queue();
                        break;
                    case "class":
                        classes.clear();
                        classIndex = 0;
                        ...
                        event.getHook().editOriginalEmbeds(getCreationEmbed().setDescription("Choose the number of healers").build()).setActionRow(classMenu).queue();
                        break;
                    case "create":
                        if(floor == null){
                            event.getHook().editOriginalEmbeds(getCreationEmbed().setDescription("You must set the floor number before creating the party").build()).queue();
                            break;
                        }
                        guildMap.get(event.getGuild().getId()).partyList.add(new Party(username, event.getUser().getId(), floor, classes, event.getChannel().getId()));
                        event.getHook().editOriginalEmbeds(defaultEmbed("Party Finder Creator").setDescription("Successfully created the party which can be joined using `/party join " + username + "`").build()).setActionRows().queue();
                        return;
                    case "cancel":
                        event.getHook().editOriginalEmbeds(defaultEmbed("Party Finder Creator").setDescription("Canceled the creation process").build()).setActionRows().queue();
                        return;
                }
                break;
            ...
            default:
                if(menuId.startsWith("party_finder_create_class_")){
                    ...
                    event.getHook().editOriginalEmbeds(getCreationEmbed().setDescription("Choose the number of " + nextClassName + "s").build()).setActionRow(nextClassMenu).queue();
                }
                break;
        }
    ...
    private SelectionMenu getMainSelectionMenu(){
       return SelectionMenu.create("party_finder_create_main")
                ...
                .addOption("Cancel", "cancel", "Cancel the party creation").build();
    }
