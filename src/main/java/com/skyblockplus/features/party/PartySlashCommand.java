	...
	protected void execute(SlashCommandExecutedEvent event) {
		...
		switch (event.getSubcommandName()) {
			case "create":
				...
				return;
			case "list":
				event.embed(PartyCommand.getPartyList(event.getGuild().getId()));
				return;
			case "leave":
				event.embed(PartyCommand.leaveParty(new PaginatorEvent(event)));
				return;
			case "disband":
				event.embed(PartyCommand.disbandParty(new PaginatorEvent(event)));
				return;
			case "join":
				event.embed(PartyCommand.joinParty(event.getOptionStr("username"), new PaginatorEvent(event)));
				return;
			case "kick":
				event.embed(PartyCommand.kickMemberFromParty(event.getOptionStr("username"), new PaginatorEvent(event)));
				return;
			case "current":
				event.embed(PartyCommand.getCurrentParty(new PaginatorEvent(event)));
				return;
			default:
