...
public class TalismanBagCommand extends Command {
          ...
          if (playerEnderChest != null) {
            ...
            if (missingEmoji.length() > 0) {
              ebMessage
                .getChannel()
                .sendMessage(
                  defaultEmbed("Missing Items")
                    .setDescription(missingEmoji)
                    .build()
                )
                .queue();
            }
            ...
            ebMessage
              .editMessage(
                defaultEmbed("Error")
                  .setDescription("Unable to fetch data")
                  .build()
              )
              .queue();
          }
  ...
  private List<String[]> getPlayerTalismansEmoji(
    String username,
    String profileName
  ) {
  ...
  private EmbedBuilder getPlayerTalismansList(
    String username,
    String profileName,
    String slotNum,
    CommandEvent event
        ...
        for (Map.Entry<Integer, InvItem> currentTalisman : talismanBagMap.entrySet()) {
          ...
          if (currentTalismanStruct == null) {
            ...
            paginateBuilder.addItems(
              "**Slot:** " + (currentTalisman.getKey() + 1)
            );
          } else {
            pageTitles.add(
              currentTalismanStruct.getName() +
              " x" +
              currentTalismanStruct.getCount()
            );
            pageThumbnails.add(
              "https://sky.lea.moe/item.gif/" + currentTalismanStruct.getId()
            );
            String itemString = "";
            ...
            itemString +=
              "\n\n**Item Creation:** " +
              currentTalismanStruct.getCreationTimestamp();
