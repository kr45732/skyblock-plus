...
public class InventoryCommand extends Command {
              ...
              ebMessage
                ...
                .sendMessage(
                  defaultEmbed("Missing Items")
                    .setDescription(playerInventory[2])
                    .build()
                )
                .queue();
            }
  ...
  private EmbedBuilder getPlayerInventoryList(
    String username,
    String profileName,
    String slotNum,
    CommandEvent event
        ...
        for (Map.Entry<Integer, InvItem> currentInvSlot : inventoryMap.entrySet()) {
          ...
          if (currentInvStruct == null) {
            ...
            paginateBuilder.addItems(
              "**Slot:** " + (currentInvSlot.getKey() + 1)
            );
          } else {
            pageTitles.add(
              currentInvStruct.getName() + " x" + currentInvStruct.getCount()
            );
            pageThumbnails.add(
              "https://sky.lea.moe/item.gif/" + currentInvStruct.getId()
            );
            String itemString = "";
            ...
            itemString +=
              "\n\n**Item Creation:** " +
              currentInvStruct.getCreationTimestamp();
  ...
  private EmbedBuilder getPlayerEquippedArmor(
    String username,
    String profileName,
    CommandEvent event
  ) {
