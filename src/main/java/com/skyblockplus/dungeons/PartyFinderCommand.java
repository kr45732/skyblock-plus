...
public class PartyFinderCommand extends Command {
    ...
    new Thread(
      () -> {
        ...
        if (args.length == 2 || args.length == 3) {
          if (args.length == 3) {
            ebMessage
              .editMessage(getPlayerDungeonInfo(args[1], args[2]).build())
              .queue();
          } else {
            ebMessage
              .editMessage(getPlayerDungeonInfo(args[1], null).build())
              .queue();
          }
          return;
        }
  ...
  private EmbedBuilder getPlayerDungeonInfo(
    String username,
    String profileName
  ) {
    ...
    if (player.isValid()) {
      ...
      eb.setDescription(
        "**Catacombs Level:** " + roundAndFormat(player.getCatacombsLevel())
      );
